...
for group in root.findall('{'+ namespace + '}' + 'PropertyGroup'):
    ...
    for element in groupElement:
        if element.tag == '{'+ namespace + '}' + 'ConfigurationType' and 'Release' in group.get('Condition'):
            element.text = element.text.replace("DynamicLibrary","StaticLibrary")
...
for group in root.findall('{'+ namespace + '}' + 'ItemDefinitionGroup'):
    ...
    for element in groupElement:
        if element.tag == '{'+ namespace + '}' + 'LinkLibraryDependencies' and 'Release' in group.get('Condition'):
            element.text = element.text.replace("false","true")
...
for group in root.findall('{'+ namespace + '}' + 'ItemDefinitionGroup'):
    ...
    for element in groupElement:
        if element.tag == '{'+ namespace + '}' + 'IgnoreSpecificDefaultLibraries' and 'Release' in group.get('Condition'):
            element.text = element.text.replace("libcmt;","")
            element.text = element.text.replace("libcpmt;","")
...
for group in root.findall('{'+ namespace + '}' + 'ItemDefinitionGroup'):
    ...
    for element in groupElement:
        if element.tag == '{'+ namespace + '}' + 'AdditionalDependencies' and 'Release' in group.get('Condition'):
            element.text = element.text.replace("advapi32.lib","advapi32.lib;legacy_stdio_definitions.lib;xapobase.lib;ucrt.lib")
...
replaceAll('xray\engine\pc\dx11\sources\engine_pc_dx11.vcxproj', 'binaries\Release\</OutDir>', r"binaries\Release\lib</OutDir>")

line_in = "'Release|x64'" + '">.dll</TargetExt>'
line_out = "'Release|x64'" + '">.lib</TargetExt>'
