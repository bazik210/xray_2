...
{
	m_model					= gcnew solid_render_model_wrapper( nullptr );
	m_matrix->identity();
	m_pivot->set			(0,0,0);
}
...
{
	if(m_model->get_render_model_())
	{
		m_parent->get_model_editor()->collision_tree->erase	( reinterpret_cast<xray::collision::object*>(m_active_collision_object) );
...
void composite_visual_item::on_model_ready( resources::queries_result& data )
{
	if ( data[0].is_successful() )
	{
		...
		*m_hq_collision_indices			= data[3].get_managed_resource();
		...
		me->collision_tree->insert		( reinterpret_cast<xray::collision::object*>(m_active_collision_object), *m_matrix );
...
void composite_visual_item::rotation::set( Float3 p )
{
	...
	set_matrix( xray::math::create_rotation( angles_rad ) * xray::math::create_translation( m_matrix->c.xyz() ) );
}
...
void composite_visual_item::on_matrix_changed( )
{
	...
	if( m_model->get_render_model_() ) 
	{
		me->collision_tree->move	( reinterpret_cast<xray::collision::object*>(m_active_collision_object), *m_matrix );
