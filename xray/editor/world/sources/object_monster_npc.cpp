...
void animation_monster_name_ui_type_editor::run_editor( xray::editor::wpf_controls::property_editors::property^ prop )
{
	wpf_controls::property_container^ container = safe_cast< wpf_controls::property_container^ >( prop->property_owners[0] );
	object_monster_npc^ npc			= safe_cast< object_monster_npc^ >( container->owner );
...
object_monster_npc::object_monster_npc	( tool_misc^ tool, render::scene_ptr const& scene ) :
	super							( tool ),
	m_parent_tool					( tool ),
...
{
	m_onframe_delegate				= gcnew execute_delegate( this, &object_monster_npc::play_animation );
	image_index						= editor_base::node_mesh;
}

object_monster_npc::~object_monster_npc	( )
{
...
void object_monster_npc::set_visible	( bool is_visible )
{
...
void object_monster_npc::set_transform( float4x4 const& transform )
{
...
void object_monster_npc::on_selected	( bool selected )
{
...
void object_monster_npc::save			( configs::lua_config_value config )
{
	super::save						( config );
	config["game_object_type"]		= "monster_npc";
...
void object_monster_npc::load_props	( configs::lua_config_value const& config )
{
...
void object_monster_npc::load_contents( )
{
}

void object_monster_npc::unload_contents( bool )
{
	if ( !m_model_instance->c_ptr() )
		return;
...
void object_monster_npc::on_model_loaded	( resources::queries_result& data )
{
...
void object_monster_npc::on_animation_loaded	( resources::queries_result& data )
{
	if ( data.is_successful() )
	{
		*m_animation = static_cast_resource_ptr< animation::skeleton_animation_ptr >( data[0].get_managed_resource() );
		m_owner_tool->get_level_editor()->add_onframe_delegate( gcnew execute_delegate( this, &object_monster_npc::play_animation ), 1 );
	}
	else
...
void object_monster_npc::load_model	( System::String^ model_name )
{
	query_result_delegate* rq		= NEW( query_result_delegate )( gcnew query_result_delegate::Delegate( this, &object_monster_npc::on_model_loaded ), g_allocator );
...
void object_monster_npc::load_animation	( System::String^ animation_name )
{
	query_result_delegate* rq			= NEW( query_result_delegate )( gcnew query_result_delegate::Delegate( this, &object_monster_npc::on_animation_loaded ), g_allocator );
	resources::query_resource			(
...
void object_monster_npc::initialize_collision( )
{
...
void object_monster_npc::play_animation	( )
{
...
void object_monster_npc::render			( )
{
	if ( !m_model_instance->c_ptr() || !m_animation->c_ptr() )
		return;
...
wpf_controls::property_container^ object_monster_npc::get_property_container	( )
{
	...
	List< String^>^ monsters = gcnew List< String^ >();
	...
	container->properties["visual/animated model instance"]->dynamic_attributes->add( 
		gcnew wpf_controls::property_editors::attributes::combo_box_items_attribute( monsters->ToArray() )
	);
