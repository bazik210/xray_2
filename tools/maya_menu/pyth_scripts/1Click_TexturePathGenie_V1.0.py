...
def populatePathList(*args):
    ...
    for tex in fileTextures:
        ...
        cmds.rowLayout('_%i_rowLayout' % (rowLayOutCount), nc=2, cw2=[
                       (int(windowWidth * .8)), (int(windowWidth * .2))], adj=1, p="uiColWrapper2")
        if (initResultPre):
            cmds.text("%s" % (tex),
                      l="%s" % (queryPath),
                      al="left",
                      bgc=tmpColorPatListPass[0],
                      w=(int(windowWidth * .5)),
                      h=pathStrHeight,
                      p='_%i_rowLayout' % (rowLayOutCount))
        elif (initResultPre == False):
            cmds.text("%s" % (tex),
                      l="%s" % (queryPath),
                      al="left",
                      bgc=tmpColorPatListFail[0],
                      w=(int(windowWidth * .5)),
                      h=pathStrHeight,
                      p='_%i_rowLayout' % (rowLayOutCount))
        cmds.button('_%i_texSelectButton' %
                    (rowLayOutCount), l="%s" %
                    (tex), w=(int(windowWidth * .4)), c='cmds.select("%s")' %
                    (tex), p='_%i_rowLayout' %
                    (rowLayOutCount))
    ...
    for tex in widgetQueryList:
        ...
        if ((initResult != True) and (len(commonLocations) != 0)):
            for commonLoc in commonLocations:
                ...
                commonPathResult = os.path.exists(
                    "%s/%s" %
                    (formattedTestPath3, searchFile))
                if (commonPathResult):
...
def browseIt():
    searchPathRaw = cmds.fileDialog2(
        fm=2,
        cap="Browse for a directory to search within..",
        dir=defaultSearchPath,
        okc="Choose Directory",
        dialogStyle=2)
...
def makeProgressBar(name, maxVal):
    ...
    progWindow = cmds.window(
        "%s" %
        (name),
        title="%s Progress Window" %
        (name),
        widthHeight=(
            300,
            50))
...
def makeGui():
    texFix_Window = cmds.window(
        'texFix_Window',
        title="Texture Search/Source Tool - V%s" %
        (version),
        iconName='texFix_Window',
        widthHeight=(
            windowWidth +
            15,
            400))
    ...
    cmds.columnLayout('uiColWrapper', w=(windowWidth - 10),
                      adj=1, adjustableColumn=True, p="scrollLayout")

    cmds.text(
        l="Automatic 1-click texture resource tool",
        bgc=[
            1,
            1,
            1],
    ...
    cmds.textFieldButtonGrp(
        'Dir',
        label="Root path for search",
        ad3=2,
        cw3=[
            105,
            380,
            50],
        text=defaultSearchPath,
        buttonLabel='browse',
        buttonCommand=browseIt,
        parent='uiColWrapper')
    cmds.button('populateResultArea', l="-Display Texture Paths-",
                bgc=[.35, .35, .35], h=40, c=populatePathList, parent='uiColWrapper')
    cmds.button('StartTextureSearch', l="-Fix Texture Paths-",
                bgc=[.35, .6, .35], h=40, en=0, c=resourceAllTextures, parent='uiColWrapper')

    cmds.separator(height=10, style='double', p='uiColWrapper')

    cmds.columnLayout('uiColWrapper2', w=(windowWidth - 10),
                      adjustableColumn=True, p="scrollLayout")
