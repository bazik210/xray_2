...
class m_meshWin(object):
    ...
    def __init__(self):
        ...
        self.m_window = cmds.window(
            m_meshProportiesGuiID,
            title='Mesh Properties',
            width=500,
            height=500)
        # create the pane
        self.m_paneLayout = cmds.paneLayout(
            parent=self.m_window,
            configuration='single')  # vertical2   <->  single
        # ----------------------
        # Left Pane
        # ----------------------
        ...
        self.m_buttonAddMesh = cmds.button(
            parent=self.m_formLayoutLeft,
            label='Add',
            width=80,
            command=lambda *args: self.uiButtonCallback(
                "m_buttonAddMesh",
                args),
            annotation="Select object in Outliner and press 'Add' button")
        self.m_buttonDelMesh = cmds.button(
            parent=self.m_formLayoutLeft,
            label='Delete',
            width=80,
            command=lambda *args: self.uiButtonCallback(
        ...
        self.m_buttonRefresh = cmds.button(
            parent=self.m_formLayoutLeft,
            label='Refresh',
            width=80,
            command=lambda *args: self.uiButtonCallback(
        ...
        # ----------------------
        # create scrollList
        self.m_textScrollList = cmds.textScrollList(
            parent=self.m_formLayoutLeft,
        ...
        self.m_textPopupMenuItemRefresh = cmds.menuItem(
            parent=self.m_textPopupMenu,
            label='Refresh',
            command=lambda *args: self.uiPopupMenuCallback(
                "m_textPopupMenuItemRefresh",
                args))
        # ----------------------
        # 3 buttons: Export All, Export Sel, Invert Sel
        self.m_buttonExportAll = cmds.button(
            parent=self.m_formLayoutLeft,
            label='Export All',
            width=80,
            command=lambda *args: self.uiButtonCallback(
        ...
        self.m_buttonExportSel = cmds.button(
            parent=self.m_formLayoutLeft,
            label='Export Sel',
            width=80,
            command=lambda *args: self.uiButtonCallback(
        ...
        self.m_buttonInvertSel = cmds.button(
            parent=self.m_formLayoutLeft,
            label='Invert Sel',
            width=80,
            command=lambda *args: self.uiButtonCallback(
        ...
        self.m_textSaveDir = cmds.text(
            parent=self.m_formLayoutLeft,
            label='Save to',
            align='left',
            width=40)
        # add textField
        self.m_textFieldSaveDir = cmds.textField(
            parent=self.m_formLayoutLeft,
            editable=False,
            changeCommand=lambda *args: self.uiSaveDirCallback(
                "m_textFieldSaveDir",
                args),
            width=150,
            annotation="Save to")
        # add symbol button
        self.m_symbolButtonSaveDir = cmds.symbolButton(
            parent=self.m_formLayoutLeft,
            image="navButtonBrowse.xpm",
            annotation="Save to: Browse for the save folder",
            command=lambda *args: self.uiSaveDirCallback(
        ...
        # create progressBar
        self.m_progressControl = cmds.progressBar(
            parent=self.m_frameLayoutInfo, maxValue=100)
        ...
        # ----------------------
        # add txt label 'Type'
        self.m_textSelectObjType = cmds.text(
            parent=self.m_formLayoutRight,
            label='Type',
            align='left',
            width=40)
        # add option menu
        self.m_optionMenuObjType = cmds.optionMenu(
            parent=self.m_formLayoutRight,
            changeCommand=lambda *args: self.uiOptionMenuCallback(
                "m_optionMenuObjType",
                args),
            annotation="Select type of the object")
        for m_item in m_objectType:
            # read options from global array m_objectType
            cmds.menuItem(parent=self.m_optionMenuObjType, label=m_item)
        # ----------------------
        # add txt label 'Tags'
        self.m_textSelectObjTags = cmds.text(
            parent=self.m_formLayoutRight,
            label='Tags',
            align='left',
            width=40)
        ...
        self.m_checkBoxTag = []
        i = 0
        while (i != len(list(m_categoryTree.keys()))):
            for key in list(m_categoryTree.keys()):
                if (i == key[0]):
                   # global hash with tags m_categoryTree
                    self.m_frameLayoutTag.append(
                        cmds.frameLayout(
                            parent=self.m_formLayoutTags,
                            label=key[1],
                            marginHeight=2,
                            marginWidth=2,
                            borderStyle='etchedIn'))
                    # global hash with tags m_categoryTree
                    for val in m_categoryTree[key]:
                        self.m_checkBoxTag.append(
                            cmds.checkBox(
                                label=val,
                                value=False,
        ...
        cmds.optionMenu(
            self.m_optionMenuObjType,
            edit=True,
            enableBackground=True)
        cmds.optionMenu(
            self.m_optionMenuObjType,
            edit=True,
            backgroundColor=m_checkBoxColor)  # single selection color
        cmds.optionMenu(
            self.m_optionMenuObjType,
            edit=True,
        ...
        for tagGroup in self.m_frameLayoutTag:
            if (i != 0):
                cmds.formLayout(self.m_formLayoutTags, edit=True, attachControl=(
                    self.m_frameLayoutTag[i], 'left', 2, self.m_frameLayoutTag[i - 1]))
            i = i + 1
        # ----------------------
        # add txt label 'Save to'
        self.m_textSaveDir = cmds.text(
    ...
    def uiButtonCallback(self, *args):
        ...
        if (m_button == 'm_buttonAddMesh'):
            m_objSelectedLong = cmds.ls(selection=True)  # get selected objects
                ...
                if (0 != len(m_items)):
                    for m_obj in m_items:
                        if (self.m_node.delObj(string.split(
                            ...
                            cmds.optionMenu(
                                self.m_optionMenuObjType, edit=True, enableBackground=True)
                            cmds.optionMenu(
                                self.m_optionMenuObjType,
                                edit=True,
                                backgroundColor=m_checkBoxColor)  # single selection color
                            cmds.optionMenu(
                                self.m_optionMenuObjType, edit=True, select=1)  # reset objType
                            cmds.optionMenu(
                                self.m_optionMenuObjType, edit=True, enableBackground=False)
                            # reset tags and backg colors
                            for m_tag_checkbox in self.m_checkBoxTag:
                                cmds.checkBox(
                                    m_tag_checkbox, edit=True, value=False)
                                cmds.checkBox(
                                    m_tag_checkbox, edit=True, enableBackground=True)
                                cmds.checkBox(
                                    m_tag_checkbox,
                                    edit=True,
                                    backgroundColor=m_checkBoxColor)  # single selection color
                                cmds.checkBox(
                                    m_tag_checkbox, edit=True, enableBackground=False)
        # -------------------------------
        #     m_buttonRefresh
        # -------------------------------
        ...
        # -------------------------------
            ...
            if (0 != self.m_node.count()):
                ...
                m_items_sel = cmds.textScrollList(
                    self.m_textScrollList,
                    query=True,
                    selectItem=True)  # get selected items from scrollList
                if (None is not m_items_sel):
                    if (0 != len(m_items_sel)):
                        m_items_unsel = []
                        cmds.textScrollList(
                            self.m_textScrollList, deselectAll=True)
                        self.m_node.setAllSelectionStates(
                            False)  # reset selection state
                        ...
                        for m_obj in m_items_unsel:  # select unselected
                            self.m_node.setSelectionState(
                                string.split(m_obj)[1], True)
                            cmds.textScrollList(
                                self.m_textScrollList, edit=True, selectItem=m_obj)
                else:
    ...
    def uiCheckBoxCallback(self, *args):
        ...
        for m_tag in self.m_checkBoxTag:
            if (cmds.checkBox(m_tag, query=True, value=True)):
                cmds.checkBox(m_tag, edit=True, enableBackground=True)
                cmds.checkBox(
                    m_tag, edit=True, backgroundColor=m_checkBoxColor)
                m_objSelTags.append(
                    getFullPathToTag(
                        cmds.checkBox(
                            m_tag,
        ...
        if (None is not m_items):
            if (0 != len(m_items)):
                for m_obj in m_items:
                    self.m_node.setObjTag(
                        string.split(m_obj)[1],
                        m_objSelTags)  # write to node
    ...
    def uiOptionMenuCallback(self, *args):
        ...
        if (m_optionMenu == 'm_optionMenuObjType'):
            ...
            cmds.optionMenu(
                ...
                edit=True,
                enableBackground=False)
            # get selected items from scrollList as string array
            m_items = cmds.textScrollList(
                self.m_textScrollList, query=True, selectItem=True)
            if (None is not m_items):
                if (0 != len(m_items)):
                    i = 0
                    while (i != len(m_items)):
                        m_obj = m_items[i]  # get old label
                        self.m_node.setObjType(
                            string.split(m_obj)[1],
                            cmds.optionMenu(
                                self.m_optionMenuObjType,
                                query=True,
                                value=True))  # write to node
                        i += 1
            self.uiRefreshScrollList()   # reload ScrollList
        ...
        # --------------------------------
            ...
            if (None is getRealXrayResourcesPath('models')
                ...
                self.errorHandle.raiseMessage(
                    "XRAY_RESOURCES_PATH/models don't exists")
            else:
                # ---------------------------------------------------------------------------------------
                # read data from node
                m_hash = self.m_node.items()
                m_items = cmds.textScrollList(
                    self.m_textScrollList,
                    query=True,
                    allItems=True)  # get all items from scrollList
                ...
                if (None is self.m_fileDialog2SaveDir):
                   # print "Cancel button pressed"
                    ...
                    # Set button pressed
                    cmds.textField(
                        self.m_textFieldSaveDir,
                        edit=True,
                        fileName=str(m_saveDir))  # write m_saveDir to textField
                    cmds.textField(
                        self.m_textFieldSaveDir,
                        edit=True,
                        enableBackground=True)
                    ...
                    if (None is not m_items):
                        if (0 != len(m_items)):
                            ...
                            while (i != len(m_items)):
                                m_obj = m_items[i]  # get old label
                                self.m_node.setSaveDir(
                                    string.split(m_obj)[1], str(
                                        m_xrayResourcesPathMeshes + m_saveDir))  # write m_saveDir to the node
                                i += 1
                    self.uiRefreshScrollList()  # reload ScrollList
    ...
    def uiLoadDataFromNode(self):
        ...
        m_items = cmds.textScrollList(
            ...
            query=True,
            allItems=True)  # get all items from scrollList
        # deselect ALL items from scrollList
        cmds.textScrollList(self.m_textScrollList, edit=True, deselectAll=True)
        m_hashSelected = {}  # create dictionary with selected items
        for obj, data in m_hash.items():
            if (data[0]):
                if (None is not m_items and 0 != len(m_items)):
        ...
        if (0 != len(m_hash)):
            ...
            for obj, data in m_hash.items():
                ...
                if (0 == m_objectType.count(m_objType)):
                    # if stored value don't exists in global array m_objectType
                    m_objType = m_objectType[0]
                else:
                    break
            ...
            if (m_hasDifference):  # if we have differences in multiselection let's color it
                ...
                cmds.optionMenu(
                    self.m_optionMenuObjType,
                    edit=True,
                    select=m_objectType.index(m_objType) + 1)
            cmds.progressBar(
                self.m_progressControl,
                edit=True,
                step=25)  # PROGRESS 50%
            ...
            m_tag_count = {}
            for m_tag_checkbox in self.m_checkBoxTag:
                m_tag_count[cmds.checkBox(
                    m_tag_checkbox, query=True, label=True)] = 0
            for obj, data in m_hash.items():
                ...
                if (None is not data[2]):  # have some tags
                    if (0 != len(data[2])):
                        for m_tag in data[2]:
                            # from 'Location|Industrial'
                            # get list of strings like this: ['Location',
                            # 'Industrial']
                            ...
                            while (i != len(list(m_categoryTree.keys()))):
                                for key in list(m_categoryTree.keys()):
                                    # if key exists in global array
                                    # m_categoryTree
                                    if ((i, m_tag_list[0]) in m_categoryTree):
                                        if (0 != m_categoryTree[(i, m_tag_list[0])].count(
                                                m_tag_list[1])):  # if tag exists in global array
                                            # increse amount by 1
                                            m_tag_count[m_tag_list[1]] += 1
                                        break
                                i += 1
            # lets set cheked tags
            for m_tag_checkbox in self.m_checkBoxTag:
                if (len(m_hash) == m_tag_count[cmds.checkBox(
                        m_tag_checkbox, query=True, label=True)]):
                    cmds.checkBox(m_tag_checkbox, edit=True, value=True)
                    cmds.checkBox(
                        m_tag_checkbox,
                        edit=True,
                        backgroundColor=m_checkBoxColor)  # all items have this tag
                    cmds.checkBox(
                        m_tag_checkbox,
                        edit=True,
                        enableBackground=True)
                if ((m_tag_count[cmds.checkBox(m_tag_checkbox, query=True, label=True)] >= 1) and (
                        m_tag_count[cmds.checkBox(m_tag_checkbox, query=True, label=True)] < len(m_hash))):
                    cmds.checkBox(m_tag_checkbox, edit=True, value=True)
                    # not all items have the same tag
                    cmds.checkBox(
                        m_tag_checkbox,
                        edit=True,
            ...
            cmds.textField(
                self.m_textFieldSaveDir,
                edit=True,
                fileName=str(m_saveDir))  # reset txtField
            cmds.progressBar(
                self.m_progressControl,
                edit=True,
                step=25)  # PROGRESS 100%
        cmds.progressBar(
            self.m_progressControl,
            edit=True,
