...
class m_animNode(object):
        ...
        if (0 == OpenMaya.MGlobal_mayaState()):
            if (not self.convertOld()):
                self.errorHandle.raiseMessage(
                    "Data convert error %s" % m_animNodeName)
        # ----------------------------------------------------------------------------------------------
    ...
    def addAttr(self):
        ...
        try:
            # add node with name 'xray_anim_lister'
            ...
            if (0 == OpenMaya.MGlobal_mayaState()):
                ...
                return False
        m_selectionList.add(m_animNodeName)
        # connect with MObject first element in the selection list
        m_selectionList.getDependNode(0, m_animNode)
        # connect MObject with function set
        m_animNodeFn.setObject(m_animNode)
        # ----------------------------------------------------------------------------------------------
        try:
            # find attribute by name 'skins'
            m_animNodeFn.attribute('_201102_skins')
        except BaseException:
            if (0 == OpenMaya.MGlobal_mayaState()):
                # ------------------
                #    ADD ATTR
                ...
                self.errorHandle.raiseMessage(
                    "%s" % OpenMaya.MFileIO.currentFile())
                self.errorHandle.raiseMessage(
                    "Found old version of data stored in %s node, convert it please" %
                    m_animNodeName)
                return False
        # ----------------------------------------------------------------------------------------------
        try:
            # find attribute by name 'animLayersMessage'
            m_animNodeFn.attribute('_201102_animLayersMessage')
        except BaseException:
            if (0 == OpenMaya.MGlobal_mayaState()):
                cmds.addAttr(
                    m_animNodeName,
                    longName='_201102_animLayersMessage',
        ...
        # ----------------------------------------------------------------------------------------------
        try:
            # find attribute by name 'clips'
            m_animNodeFn.attribute('_201102_clips')
        except BaseException:
            if (0 == OpenMaya.MGlobal_mayaState()):
    ...
    def convertOld(self):
        ...
        try:
            ...
            #  from201101to201102
            # ------------------------------------------
            m_animNodeFn.attribute('_201101_skins')
            m_animNodeFn.attribute('_201101_clips')
        except BaseException:
            ...
            try:
                ...
                m_animNodePlugSkins = OpenMaya.MPlug(
                    m_animNode, m_animNodeAttrSkins)
                m_animNodePlugSkins.getExistingArrayAttributeIndices(
                    m_indexesSkins)
            except BaseException:
                return False
            for m_index in m_indexesSkins:
                # GET
                m_skinName = cmds.getAttr(
                    "%s._201101_skins[%s]._201101_skinNameSkins" %
                    (m_animNodeName, m_index))
                m_isSkinSel = cmds.getAttr(
                    "%s._201101_skins[%s]._201101_isSkinSel" %
                    (m_animNodeName, m_index))
                # SET
            ...
            try:
                ...
                m_animNodePlugClips = OpenMaya.MPlug(
                    m_animNode, m_animNodeAttrClips)
                m_animNodePlugClips.getExistingArrayAttributeIndices(
                    m_indexesClips)
            except BaseException:
                return False
            for m_index in m_indexesClips:
                # GET
                m_skinName = cmds.getAttr(
                    "%s._201101_clips[%s]._201101_skinNameClips" %
                    (m_animNodeName, m_index))
                m_animName = cmds.getAttr(
                    "%s._201101_clips[%s]._201101_animName" %
                    (m_animNodeName, m_index))
                m_isAnimSel = cmds.getAttr(
                    "%s._201101_clips[%s]._201101_isAnimSel" %
                    (m_animNodeName, m_index))
                m_startFrame = cmds.getAttr(
                    "%s._201101_clips[%s]._201101_startFrame" %
                    (m_animNodeName, m_index))
                m_endFrame = cmds.getAttr(
                    "%s._201101_clips[%s]._201101_endFrame" %
                    (m_animNodeName, m_index))
                m_localCoordCenter = cmds.getAttr(
                    "%s._201101_clips[%s]._201101_LCC" %
                    (m_animNodeName, m_index))
                m_FPS = cmds.getAttr(
                    "%s._201101_clips[%s]._201101_FPS" %
                    (m_animNodeName, m_index))
                m_saveDirIsDefault = cmds.getAttr(
                    "%s._201101_clips[%s]._201101_saveDirIsDefault" %
                    (m_animNodeName, m_index))
                m_epsilonIsDefault = cmds.getAttr(
                    "%s._201101_clips[%s]._201101_epsilonIsDefault" %
                    (m_animNodeName, m_index))
                if (m_saveDirIsDefault):
                    m_saveDir = m_anim_saveDirDefault
                else:
                    m_saveDir = cmds.getAttr(
                        "%s._201101_clips[%s]._201101_saveDir" %
                        (m_animNodeName, m_index))
                if (m_epsilonIsDefault):
                    m_epsilon = m_anim_epsilonDefault
                else:
                    m_epsilon = cmds.getAttr(
                        "%s._201101_clips[%s]._201101_epsilon" %
                        (m_animNodeName, m_index))
            ...
            if (not m_animNode):
                return False
            if (0 != len(m_skins)):
                for obj, data in m_skins.items():
                    m_index = obj
                    m_skinName = data[0]
                    m_isSkinSel = bool(data[1])
                    cmds.setAttr(
                        '%s._201102_skins[%s]._201102_skinNameSkins' %
                        (m_animNodeName, m_index), m_skinName, type='string')
                    cmds.setAttr(
                        '%s._201102_skins[%s]._201102_isSkinSel' %
                        (m_animNodeName, m_index), m_isSkinSel)
            if (0 != len(m_clips)):
                for obj, data in m_clips.items():
                    m_index = obj
                    m_skinName = data[0]
                    m_animName = data[1]
                    ...
                    cmds.setAttr(
                        '%s._201102_clips[%s]._201102_skinNameClips' %
                        (m_animNodeName, m_index), m_skinName, type='string')
                    cmds.setAttr(
                        '%s._201102_clips[%s]._201102_animName' %
                        (m_animNodeName, m_index), m_animName, type='string')
                    cmds.setAttr(
                        '%s._201102_clips[%s]._201102_isAnimSel' %
                        (m_animNodeName, m_index), m_isAnimSel)
                    cmds.setAttr(
                        '%s._201102_clips[%s]._201102_startFrame' %
                        (m_animNodeName, m_index), m_startFrame)
                    cmds.setAttr(
                        '%s._201102_clips[%s]._201102_endFrame' %
                        (m_animNodeName, m_index), m_endFrame)
                    cmds.setAttr(
                        '%s._201102_clips[%s]._201102_LCC' %
                        (m_animNodeName, m_index), m_localCoordCenter, type='string')
                    cmds.setAttr(
                        '%s._201102_clips[%s]._201102_saveDir' %
                        (m_animNodeName, m_index), m_saveDir, type='string')
                    cmds.setAttr(
                        '%s._201102_clips[%s]._201102_saveDirIsDefault' %
                        (m_animNodeName, m_index), m_saveDirIsDefault)
                    cmds.setAttr(
                        '%s._201102_clips[%s]._201102_FPS' %
                        (m_animNodeName, m_index), m_FPS)
                    cmds.setAttr(
                        '%s._201102_clips[%s]._201102_epsilon' %
                        (m_animNodeName, m_index), m_epsilon)
                    cmds.setAttr(
                        '%s._201102_clips[%s]._201102_epsilonIsDefault' %
                        (m_animNodeName, m_index), m_epsilonIsDefault)
                    cmds.setAttr(
                        '%s._201102_clips[%s]._201102_isAdditive' %
                        (m_animNodeName, m_index), m_isAdditive)
                    cmds.setAttr(
                        '%s._201102_clips[%s]._201102_baseAnimation' %
                        (m_animNodeName, m_index), m_baseAnimation, type='string')
                    cmds.setAttr(
                        '%s._201102_clips[%s]._201102_baseAnimationIsDefault' %
                        (m_animNodeName, m_index), m_baseAnimationIsDefault)
        return True
    ...
    def getIndexes(self):
        ...
        if (bool(m_animNode)):
            m_animNodeFn = OpenMaya.MFnDependencyNode(
                m_animNode)  # create a function set
            ...
            m_animNodePlugSkins = OpenMaya.MPlug(
                m_animNode, m_animNodeAttrSkins)  # create a MPlug
            m_animNodePlugClips = OpenMaya.MPlug(
                m_animNode, m_animNodeAttrClips)  # create a MPlug
            m_animNodePlugSkins.getExistingArrayAttributeIndices(
                m_indexesSkins)
            m_animNodePlugClips.getExistingArrayAttributeIndices(
                m_indexesClips)
        return (
            m_indexesSkins.length(),
            m_indexesSkins,
            m_indexesClips.length(),
            m_indexesClips)
    ...
    def getConnectedAnimLayers(self):
        ...
        try:
            ...
            for m_ind in m_indexes:
                m_plug_children_layers = m_nodePlug.elementByLogicalIndex(
                    m_ind)
                connectedPlugs = OpenMaya.MPlugArray()
                if (m_plug_children_layers.connectedTo(
                        connectedPlugs, True, False)):
                    m_plug_destination = connectedPlugs[0]
                    m_obj_array.append(
                        m_plug_destination.node())  # append MObject to array
                else:
                    cmds.removeMultiInstance(
                        '%s._201102_animLayersMessage[%d]' %
                        (m_animNodeName, m_ind))
        except BaseException:
            return None
        ...
        for m_ind in range(m_obj_array.length()):
            if (not self.isAnimLayerAlreadyConnected(m_obj_array[m_ind])):
                ...
                try:
                    ...
                    self.errorHandle.raiseMessage(
                        "can't find %s" % m_animNodeName)
                    return False
                try:
                    m_nodeAttr = m_nodeFn.attribute(
                        '_201102_animLayersMessage')
                    m_nodePlug = OpenMaya.MPlug(m_node, m_nodeAttr)
                except BaseException:
                    self.errorHandle.raiseMessage(
                        "can't find %s._201102_animLayersMessage" %
                        m_animNodeName)
                    return False
                m_indexes = OpenMaya.MIntArray()
                try:
                    ...
                    self.errorHandle.raiseMessage(
                        "wrong type %s._201102_animLayersMessage should be 'message' array" %
                        m_animNodeName)
                    return False
                ...
                src = "%s.message" % getNodeNameFromeMObject(
                    m_obj_array[m_ind])
                dst = "%s._201102_animLayersMessage[%d]" % (
                    m_animNodeName, m_nextAvailable)
                # print ("connect %s to %s " %( src, dst ) )
                try:
                    cmds.connectAttr(src, dst)
                except BaseException:
                    self.errorHandle.raiseMessage(
                        "can't connect attr %s to %s " %
                        (src, dst))
                    return False
        return True
    ...
    def animLayerNodesSynchronize(self):
        ...
        try:
            ...
            self.errorHandle.raiseMessage(
                "can't find %s._201102_animLayersMessage or attr has wrong type (should be array of 'message')" %
                m_animNodeName)
            return False
        try:
            ...
            self.errorHandle.raiseMessage(
                "can't find %s._201102_clips or attr has wrong type" %
                m_animNodeName)
            return False
        # ------------------------------------------------------------
        if (0 == OpenMaya.MGlobal_mayaState()):  # MAIN PROGRESS BAR
            ...
            cmds.progressBar(
                gMainProgressBar,
                edit=True,
                beginProgress=True)  # BEGIN PROGRESS
        # ------------------------------------------------------------
        for m_indAL in m_indexesClips:
            # indexes of _201102_animLayers attribute for current _201102_clips
            # item
            m_indexesAnimLayers = OpenMaya.MIntArray()
            # child(14) = 14 is the index of _201102_animLayers attribute
            m_animNodePlugClips.elementByLogicalIndex(m_indAL).child(
                14).getExistingArrayAttributeIndices(m_indexesAnimLayers)
            ...
            if (0 != m_indexes.length()):
                for m_ind in range(m_indexes[m_indexes.length() - 1] + 1):
                    # source xray_anim_lister._201102_animLayersMessage[%d]
                    m_a = findMIntArray(m_indexes, m_ind)
                    # destination
                    # xray_anim_lister._201102_clips[%d]._201102_animLayers[%d]
                    m_b = findMIntArray(m_indexesAnimLayers, m_ind)
                    if (m_a and m_b):
                        pass
                        # print "Ok"
                    elif (m_a and not m_b):
                        cmds.getAttr(
                            '%s._201102_clips[%d]._201102_animLayers[%d]' %
                            (m_animNodeName, m_indAL, m_ind))
    ...
    def animLayerNodesFindIndex(self, m_find_node_name):
        ...
        if (m_obj_tuple):
            for m_ind in range(m_obj_tuple[1].length()):
                # print "%s %s" %( m_obj_tuple[1][m_ind],
                # getNodeNameFromeMObject( m_obj_tuple[0][m_ind] ) )
                if (m_find_node_name == getNodeNameFromeMObject(
                        m_obj_tuple[0][m_ind])):
    ...
    def alreadyExistsSkin(self, m_skinName):
        ...
        m_animNode = self.getNode()  # MObject
        if (bool(m_animNode)):
            # Result: ( skins_count , [skins_indexes], clips_count, [clips_indexes] )
            m_indexes = self.getIndexes()
            for m_index in m_indexes[1]:
                if (m_skinName == cmds.getAttr(m_animNodeName +
                                               '._201102_skins[' + str(m_index) + ']._201102_skinNameSkins')):
                    return True
        return False
    ...
    def addSkin(self, m_skinName, m_isSkinSel=False):
        ...
        """
        m_animNode = self.getNode()  # MObject
        if (self.alreadyExistsSkin(m_skinName)
            ):    # check if skin alreadyExists
            return False
        else:
            if (bool(m_animNode)):
                ...
                if (0 == m_indexes[0]):
                    ...
                    # --------------------------------
                cmds.getAttr(
                    m_animNodeName + '._201102_skins[' + str(m_index) + ']',
                    type=True)  # Create a plug by accessing iter
                # ---------------------------------
                # ----------- unlcok --------------
                cmds.setAttr(
                    m_animNodeName +
        ...
        if (bool(m_animNode)):
            if (self.alreadyExistsSkin(m_skinName)):    # check if skin Exists
                ...
                if (0 == m_indexes[2]):
                    ...
                    # --------------------------------
                cmds.getAttr(
                    m_animNodeName + '._201102_clips[' + str(m_index) + ']',
                    type=True)  # Create a plug by accessing iter
                # ---------------------------------
                # ----------- unlcok --------------
                cmds.setAttr(
                ...
                cmds.setAttr(
                    ...
                    '._201102_clips[' +
                    str(m_index) +
                    ']._201102_baseAnimation',
                    lock=False)
                cmds.setAttr(
                    m_animNodeName +
                    '._201102_clips[' +
                    str(m_index) +
                    ']._201102_baseAnimationIsDefault',
                    lock=False)
                cmds.setAttr(
                    m_animNodeName +
                    '._201102_clips[' +
            ...
            if (self.alreadyExistsSkin(m_skinName)):    # check if skin Exists
                ...
                for m_index in m_indexes[3]:
                    if (m_index == m_clipID):
                        # ----------- unlcok --------------
                        cmds.setAttr(
                            m_animNodeName + '._201102_clips[' + str(m_index) + ']', lock=False)
                        cmds.setAttr(
                            m_animNodeName +
                            '._201102_clips[' +
                            str(m_index) +
    ...
    def delSkin(self, m_skinName):
        ...
        if (bool(m_animNode)):
            if (self.alreadyExistsSkin(m_skinName)):    # check if skin Exists
                ...
                # ---------------------------------------------------------------------------------------------
                for m_index in m_indexes[3]:
                    if (m_skinName == cmds.getAttr(
                            m_animNodeName + '._201102_clips[' + str(m_index) + ']._201102_skinNameClips')):
                        # ----------- unlcok --------------
                        cmds.setAttr(
                            m_animNodeName + '._201102_clips[' + str(m_index) + ']', lock=False)
                        cmds.setAttr(
                ...
                for m_index in m_indexes[1]:
                    if (m_skinName == cmds.getAttr(
                            m_animNodeName + '._201102_skins[' + str(m_index) + ']._201102_skinNameSkins')):
                        # ----------- unlcok --------------
                        cmds.setAttr(
                            m_animNodeName + '._201102_skins[' + str(m_index) + ']', lock=False)
                        cmds.setAttr(
                            m_animNodeName +
                            '._201102_skins[' +
                            str(m_index) +
    ...
    def printAnims(self):
        ...
        if (bool(m_animNode)):
            ...
            for m_index in m_indexes[3]:
                self.errorHandle.raiseMessage(
                    "SKIN: %s <==> ANIM: %s SEL: %s MIN: %s MAX: %s LCC: %s DIR: %s %s FPS: %s EPS: %s %s %s %s %s" % (cmds.getAttr(
                        m_animNodeName + '._201102_clips[' + str(m_index) + ']._201102_skinNameClips'),
                        cmds.getAttr(
                        m_animNodeName + '._201102_clips[' + str(m_index) + ']._201102_animName'),
                        cmds.getAttr(
                        m_animNodeName + '._201102_clips[' + str(m_index) + ']._201102_isAnimSel'),
                        cmds.getAttr(
                        m_animNodeName + '._201102_clips[' + str(m_index) + ']._201102_startFrame'),
                        cmds.getAttr(
                        m_animNodeName + '._201102_clips[' + str(m_index) + ']._201102_endFrame'),
                        cmds.getAttr(
                        m_animNodeName + '._201102_clips[' + str(m_index) + ']._201102_LCC'),
                        cmds.getAttr(
                        m_animNodeName + '._201102_clips[' + str(m_index) + ']._201102_saveDir'),
                        cmds.getAttr(
                        m_animNodeName + '._201102_clips[' + str(m_index) + ']._201102_saveDirIsDefault'),
                        cmds.getAttr(
                        m_animNodeName + '._201102_clips[' + str(m_index) + ']._201102_FPS'),
                        cmds.getAttr(
                        m_animNodeName + '._201102_clips[' + str(m_index) + ']._201102_epsilon'),
                        cmds.getAttr(
                        m_animNodeName + '._201102_clips[' + str(m_index) + ']._201102_epsilonIsDefault'),
                        cmds.getAttr(
                        m_animNodeName + '._201102_clips[' + str(m_index) + ']._201102_isAdditive'),
                        cmds.getAttr(
                        m_animNodeName + '._201102_clips[' + str(m_index) + ']._201102_baseAnimation'),
                        cmds.getAttr(
                        m_animNodeName + '._201102_clips[' + str(m_index) + ']._201102_baseAnimationIsDefault')))
        ...
        if (bool(m_animNode)):
            ...
            for m_index in m_indexes[3]:
                if (m_index == m_clipID):
                    ...
                    cmds.setAttr(
                        '%s._201102_clips[%d]' %
                        (m_animNodeName, m_index), lock=False)
                    cmds.setAttr(
                        '%s._201102_clips[%d]._201102_isAdditive' %
                        (m_animNodeName, m_index), lock=False)
                    # ------------- set ---------------
                    cmds.setAttr(
                        '%s._201102_clips[%d]._201102_isAdditive' %
                        (m_animNodeName, m_index), bool(m_isAdditive))
                    # ------------ lock ---------------
                    cmds.setAttr(
                        '%s._201102_clips[%d]' %
                        (m_animNodeName, m_index), lock=True)
                    cmds.setAttr(
                        '%s._201102_clips[%d]._201102_isAdditive' %
                        (m_animNodeName, m_index), lock=True)
                    return True
            ...
            for m_index in m_indexes[3]:
                if (m_index == m_clipID):
                    ...
                    cmds.setAttr(
                        ...
                        (m_animNodeName, m_index), lock=False)
                    cmds.setAttr(
                        '%s._201102_clips[%d]._201102_baseAnimation' %
                        (m_animNodeName, m_index), lock=False)
                    cmds.setAttr(
                        '%s._201102_clips[%d]._201102_baseAnimationIsDefault' %
                        (m_animNodeName, m_index), lock=False)
                    # ------------- set ---------------
                    cmds.setAttr(
                        '%s._201102_clips[%d]._201102_baseAnimation' %
            ...
            for m_index in m_indexes[3]:
                if (m_index == m_clipID):
                    ...
                    cmds.setAttr(
                        ...
                        (m_animNodeName, m_index), lock=False)
                    cmds.setAttr(
                        '%s._201102_clips[%d]._201102_animLayers' %
                        (m_animNodeName, m_index), lock=False)
                    # ------------- set ---------------
                    for obj, data in m_checkboxStates.items():
                        m_current_anim_layer_index = self.animLayerNodesFindIndex(
                            obj)
                        m_ind = m_current_anim_layer_index[1]
                        cmds.setAttr(
                            '%s._201102_clips[%d]._201102_animLayers[%d]' %
                            (m_animNodeName, m_index, m_ind), lock=False)
                        cmds.setAttr(
                            '%s._201102_clips[%d]._201102_animLayers[%d]' %
                            (m_animNodeName, m_index, m_ind), data)
                    # ------------ lock ---------------
                    cmds.setAttr(
                        '%s._201102_clips[%d]' %
                        (m_animNodeName, m_index), lock=True)
    ...
    def setClipName(self, m_clipID, m_animName=''):
        ...
        """
        m_animName = makeName(m_animName,
                              'filename')      # format string for filename
        m_animNode = self.getNode()  # MObject
        if (bool(m_animNode)):
            # Result: ( skins_count ,  [skins_indexes],  clips_count,   [clips_indexes] )
            m_indexes = self.getIndexes()
    ...
    def setClipLocalCoordCenter(self, m_clipID, m_localCoordCenter=''):
        ...
        """
        # m_localCoordCenter = makeName( m_localCoordCenter, 'nodename' )
        # # format string for nodename
        m_animNode = self.getNode()  # MObject
        if (bool(m_animNode)):
            # Result: ( skins_count ,  [skins_indexes],  clips_count,   [clips_indexes] )
            m_indexes = self.getIndexes()
    ...
    def setClipFPS(self, m_clipID, m_FPS):
        ...
        if (bool(m_animNode)):
            ...
            if (0 == m_indexes[2]):
                ...
                for m_index in m_indexes[3]:
                    if (m_index == m_clipID):
                        m_FPSOld = cmds.getAttr(
                            m_animNodeName + '._201102_clips[' + str(m_index) + ']._201102_FPS')
                        m_startFrameOld = cmds.getAttr(
                            m_animNodeName + '._201102_clips[' + str(m_index) + ']._201102_startFrame')
                        m_endFrameOld = cmds.getAttr(
                            m_animNodeName + '._201102_clips[' + str(m_index) + ']._201102_endFrame')
                        OpenMaya.MTime.setUIUnit(m_FPSOld)
                        m_startFrame = m_startFrameOld * \
                            OpenMaya.MTime.asUnits(OpenMaya.MTime(), m_FPS)
                        m_endFrame = m_endFrameOld * \
                            OpenMaya.MTime.asUnits(OpenMaya.MTime(), m_FPS)
                        # ----------- unlcok --------------
                        cmds.setAttr(
                            m_animNodeName + '._201102_clips[' + str(m_index) + ']', lock=False)
                        ...
                        # ------------ lock ---------------
                        cmds.setAttr(
                            m_animNodeName + '._201102_clips[' + str(m_index) + ']', lock=True)
                        cmds.setAttr(
                            m_animNodeName +
                            '._201102_clips[' +
                            str(m_index) +
    ...
    def setSkinName(self, m_skinName, m_skinNameNew):
        ...
        if (self.alreadyExistsSkin(m_skinNameNew)
            ):    # check if new skin alreadyExists
            return False
        if (bool(m_animNode)):
            if (self.alreadyExistsSkin(m_skinName)
                ...
                for m_index in m_indexes[1]:
                    if (m_skinName == cmds.getAttr(
                        ...
                        # ----------- lcok ----------------
                        cmds.setAttr(
                            m_animNodeName + '._201102_skins[' + str(m_index) + ']', lock=True)
                        cmds.setAttr(
                            m_animNodeName +
                            '._201102_skins[' +
                            str(m_index) +
    ...
    def iterItemsSkin(self):
        ...
        # ------------------------------------------------------------
        m_returnHash = {}
        # [ maxLen(getShortName(m_skinName)), maxLen(m_skinName) ]
        m_maxLenght = self.getMaxLenListSkin()
        m_animNode = self.getNode()  # MObject
        if (bool(m_animNode)):
            # Result: ( skins_count ,  [skins_indexes],  clips_count,   [clips_indexes] )
            m_indexes = self.getIndexes()
            ...
            for m_index in m_indexes[1]:
                m_skinName = cmds.getAttr(
                    ...
                    ']._201102_skinNameSkins')
                m_isSkinSel = cmds.getAttr(
                    m_animNodeName + '._201102_skins[' + str(m_index) + ']._201102_isSkinSel')
                # -----------------------------
                m_list = []
                m_list.append(addSpacesToString(getShortName(
                    m_skinName), m_maxLenght[0] - len(getShortName(m_skinName))))  # stalker
                m_list.append(
                    addSpacesToString(
                        m_skinName,
                        m_maxLenght[1] -
                        len(m_skinName)))                # |group1|stalker
                # 'stalker    |group1|stalker'
                # -------------------------------
                m_returnHash[m_skinName] = (
                    bool(m_isSkinSel), string.join(
                        m_list, '    '))
                # ------------------------------------------------------------
                if (0 == OpenMaya.MGlobal_mayaState()
                        ):     # INCREASE PROGRESS STEP
    ...
    def iterItemsClip(self, m_stat='all', m_selSkinName=None):
        ...
        if (bool(m_animNode)):
            ...
            for m_index in m_indexes[3]:
                ...
                m_clipID = m_index
                # -------------------------------
                m_isAdditive = cmds.getAttr(
                    '%s._201102_clips[%d]._201102_isAdditive' %
                    (m_animNodeName, m_index))
                m_baseAnimationIsDefault = cmds.getAttr(
                    '%s._201102_clips[%d]._201102_baseAnimationIsDefault' %
                    (m_animNodeName, m_index))
                if (m_baseAnimationIsDefault):
                    m_baseAnimation = m_anim_baseAnimationDefault
                else:
                    m_baseAnimation = cmds.getAttr(
                        '%s._201102_clips[%d]._201102_baseAnimation' %
                        (m_animNodeName, m_index))
                m_animLayers = cmds.getAttr(
                    '%s._201102_clips[%d]._201102_animLayers' %
                    (m_animNodeName, m_index))
                # m_xrayResourcesPathAnims = 'XRAY_RESOURCES_PATH/animations'
                m_saveDir = m_saveDir.replace('XRAY_RESOURCES_PATH', '', 1)
                m_baseAnimation = m_baseAnimation.replace(
                    'XRAY_RESOURCES_PATH', '', 1)
                m_animFilename = "%s/%s.clip" % (m_saveDir, m_animName)
                m_baseAnimFilename = "%s.clip" % m_baseAnimation
                # -------------------------------------------------------------
                m_list = []
                m_list.append(addSpacesToString(
                    str(m_clipID), m_maxLenght[7] - len(str(m_clipID))))             # 3
                m_list.append(
                    addSpacesToString(
                        m_animName,
                        m_maxLenght[2] -
                        len(m_animName)))                # run_turn_left
                m_list.append(
                    addSpacesToString(
                        m_startFrameStr,
                        m_maxLenght[3] -
                        len(m_startFrameStr)))           # 0
                m_list.append(
                    addSpacesToString(
                        m_endFrameStr,
                        m_maxLenght[4] -
                        len(m_endFrameStr)))             # 24
                m_list.append(
                    addSpacesToString(
                        m_animFilename,
                        m_maxLenght[6] -
                        len(m_animFilename)))            # /animations/actors/stalker/run
                m_list.append(
                    addSpacesToString(
                        m_localCoordCenter,
                        m_maxLenght[5] -
                        len(m_localCoordCenter)))        # pCube1
                m_list.append(
                    addSpacesToString(
                        m_FPSStr,
                        m_maxLenght[0] -
                        len(m_FPSStr)))                  # NTSC (30 fps)
                m_list.append(
                    addSpacesToString(
                        m_epsilonStr,
                        m_maxLenght[1] -
                        len(m_epsilonStr)))              # 0.005
                if (m_isAdditive):
                ...
                if ('all' == m_stat):
                    m_returnHash[m_clipID] = (
                        ...
                        m_isAdditive,
                        m_baseAnimFilename,
                        m_baseAnimationIsDefault,
                        m_animLayers)
                elif ('sel' == m_stat):
                    if (None is not m_selSkinName and m_skinName == m_selSkinName):
                        m_returnHash[m_clipID] = (
                            m_skinName,
                            bool(m_isAnimSel),
                            m_animName,
    ...
    def exportData(
        ...
        if (None is not m_skinNameSelected):
            if (not self.alreadyExistsSkin(m_skinNameSelected)):
                m_skinNameSelected = None
        m_startFrameOriginal = float(
            cmds.playbackOptions(
                query=True, min=True))
        m_endFrameOriginal = float(cmds.playbackOptions(query=True, max=True))
        m_FPSOriginal = OpenMaya.MTime_uiUnit()
        m_anim_layers = m_animLayers()  # class for Animation Layer nodes
        # get snapshot of .Mute attr from all animLayer nodes
        m_anim_layers.animLayersMuteSnapshot()
        if ('debug' == m_mode):
            self.errorHandle.raiseMessage("ProgressBar section")
        # ------------------------------------------------------------
        if (0 == OpenMaya.MGlobal_mayaState()):  # MAIN PROGRESS BAR
            gMainProgressBar = mel.eval('$tmp = $gMainProgressBar')
            cmds.progressBar(gMainProgressBar,
        ...
        if (bool(m_animNode)):
            ...
            for m_index in m_indexes[3]:
                if ('debug' == m_mode):
                    self.errorHandle.raiseMessage("Get Attributes")
                m_skinName = cmds.getAttr(
                    m_animNodeName +
                    '._201102_clips[' +
                    str(m_index) +
                ...
                if (m_baseAnimationIsDefault):
                    ...
                    m_baseAnimation = cmds.getAttr(
                        '%s._201102_clips[%d]._201102_baseAnimation' %
                        (m_animNodeName, m_index))
                m_animLayersAttr = cmds.getAttr(
                    '%s._201102_clips[%d]._201102_animLayers' %
                    (m_animNodeName, m_index))
                m_animLayersAttr = m_animLayersAttr[0]
                m_baseAnimation = m_baseAnimation.replace(
                    m_xrayResourcesPathAnims, '', 1)
                if ('debug' == m_mode):
                    self.errorHandle.raiseMessage("Create directories")
                # try to create directories
                if (not createDirectories(getRealXrayResourcesPath(
                        'animations/single'), m_saveDir)):
                    self.errorHandle.raiseMessage(
                        "ERROR   == > Can't create directory %s" % m_saveDir)
                    break  # stop processing export current animation item
                # ---------------------------------------------------------------------------------------------
                if ('debug' == m_mode):
                    self.errorHandle.raiseMessage("Get filenames")
                m_expFilename = getRealXrayResourcesPath(
                    'animations/single') + m_saveDir + '/' + m_animName
                m_expFilename = m_expFilename.lower()
                m_baseAnimFilename = getRealXrayResourcesPath(
                    'animations/single') + m_baseAnimation
                # ---------------------------------------------------------------------------------------------
                m_ruleSel = False
                if (m_onlySelected):  # export only selected
                    if (None is not m_skinNameSelected and m_skinNameSelected ==
                            m_skinName and m_isAnimSel):
                        m_ruleSel = True
                else:
                    m_ruleSel = True
                m_ruleAdd = True
                ...
                if (m_ruleSel and m_ruleAdd):
                    if (not cmds.objExists(m_skinName)):
                        self.errorHandle.raiseMessage(
                            "ERROR   == > Can't find object %s in scene" % m_skinName)
                    elif (not isMeshHasSkinCluster(m_skinName)):
                        self.errorHandle.raiseMessage(
                            "ERROR   == > Can't find skinCluster %s" % m_skinName)
                    else:
                        if ('debug' == m_mode):
                            self.errorHandle.raiseMessage("Reset Scene")
                            ...
                            if (m_obj_array):
                                for m_ind in range(m_obj_array.length()):
                                    if (m_animLayersAttr):
                                        ...
                                        m_mute_state = False
                                        self.errorHandle.raiseMessage(
                                            "WARNING   == > may contain incorrect animation %s" % m_expFilename)
                                    cmds.animLayer(
                                        getNodeNameFromeMObject(
                                            m_obj_array[m_ind]),
                                        edit=True,
                                        mute=m_mute_state)
                        # ---------------------
                        ...
                        if (m_isAdditive):
                            try:
                                cmds.xray_anim_export(
                                    outfilename=m_expFilename,
                                    additiveToAnimation=m_baseAnimFilename,
                                    precision_t=m_epsilon,
                                    precision_r=m_epsilon,
                                    precision_s=m_epsilon)
                        ...
                        # ---------------------
                            ...
                            if (cmds.objExists(m_localCoordCenter)):
                                try:
                                    cmds.xray_anim_export(
                                        outfilename=m_expFilename,
                                        localCoordinatCenterNode=m_localCoordCenter,
                                        precision_t=m_epsilon,
                                        precision_r=m_epsilon,
                                        precision_s=m_epsilon)
                                ...
                                self.errorHandle.raiseMessage(
                                    "ERROR   == > Can't find local_center %s in %s" %
                                    (m_localCoordCenter, m_expFilename))
                                m_erroNumber = 4
                        # ---------------------
                        #      ANIM
                        # ---------------------
...
def animLayerArrayNormalize():
    ...
    if (bool(m_animNode)):
        ...
        m_animNodeAttrClips = m_animNodeFn.attribute('_201102_clips')
        m_animNodeAttrMessages = m_animNodeFn.attribute(
            '_201102_animLayersMessage')
        m_animNodePlugMessages = OpenMaya.MPlug(
            m_animNode, m_animNodeAttrMessages)
        m_animNodePlugClips = OpenMaya.MPlug(
            m_animNode, m_animNodeAttrClips)  # create a MPlug
        m_animNodePlugClips.getExistingArrayAttributeIndices(m_indexesClips)
        m_animNodePlugMessages.getExistingArrayAttributeIndices(
            m_indexesMessages)
    for m_index in m_indexesClips:
        if (m_indexesMessages):
            for m_ind in m_indexesMessages:
                m_currentState = cmds.getAttr(
                    '%s._201102_clips[%d]._201102_animLayers[%d]' %
                    (m_animNodeName, m_index, m_ind))
                if (not m_currentState):
                    ...
                    cmds.setAttr(
                        '%s._201102_clips[%d]._201102_animLayers[%d]' %
                        (m_animNodeName, m_index, m_ind), False)
                    # ------------ lock ---------------
                    cmds.setAttr(
                        '%s._201102_clips[%d]' %
                        (m_animNodeName, m_index), lock=True)
