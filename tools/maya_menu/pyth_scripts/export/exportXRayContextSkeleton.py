...
class m_skeletonNode(object):
        ...
        while not m_dagIterator.isDone():  # iterate
            ...
            if (m_objCurr.hasFn(OpenMaya.MFn.kTransform)):
                ...
                if (not m_nodeFn.isFromReferencedFile()):
                    try:
                        m_nodeFn.attribute('_201102_skel_skeletons')
                        m_nodeFn.attribute('_201102_skel_isSel')
                        m_nodeFn.attribute('_201102_skel_saveDir')
                        m_nodeFn.attribute('_201102_skel_saveDirIsDefault')
    ...
    def addObj(self, m_obj):
        ...
        try:
            m_nodeFn.attribute('_201102_skel_skeletons')
            self.errorHandle.raiseMessage(
                "Attribute %s._201102_skel_skeletons already exists" %
                m_nodeFn.name())
            return False
        except BaseException:
            pass
        ...
        try:
            ...
            cmds.addAttr(
                m_nodeName,
                parent='_201102_skel_skeletons',
                longName='_201102_skel_isSel',
                niceName='Is Selected',
                attributeType='bool')
            cmds.addAttr(
                m_nodeName,
                parent='_201102_skel_skeletons',
                longName='_201102_skel_saveDir',
                niceName='Save to',
                dataType='string')
            cmds.addAttr(
                m_nodeName,
                parent='_201102_skel_skeletons',
                longName='_201102_skel_saveDirIsDefault',
                niceName='Save to Is Default',
                attributeType='bool')
        except BaseException:
            self.errorHandle.raiseMessage(
                "Can't add attribute '_201102_skel_skeletons' to node %s" %
                m_nodeName)
            return False
        # ----------- unlock --------------
        ...
        cmds.setAttr(
            '%s._201102_skel_saveDirIsDefault' %
            m_nodeName, lock=False)
        # ------------- set ---------------
        cmds.setAttr('%s._201102_skel_isSel' % m_nodeName, bool(m_isSelected))
        cmds.setAttr(
            '%s._201102_skel_saveDir' %
            m_nodeName,
            m_saveDir,
            type='string')
        cmds.setAttr(
            '%s._201102_skel_saveDirIsDefault' %
            m_nodeName, bool(m_saveDirIsDefault))
        # ------------ lock ---------------
    ...
    def delObj(self, m_obj):
        ...
        try:
            ...
            self.errorHandle.raiseMessage(
                "Can't find %s._201102_skel_skeletons attribute" %
                m_nodeName)
        return False
    ...
    def printObj(self):
        ...
        while not m_listIt.isDone():
            ...
            try:
                ...
                m_nodeFn.attribute('_201102_skel_saveDirIsDefault')
                self.errorHandle.raiseMessage(
                    "KEY: %s <==> SEL: %s DIR: %s DID %s" %
                    (m_nodeName,
                     cmds.getAttr(
                         '%s._201102_skel_isSel' %
                         m_nodeName),
                        cmds.getAttr(
                         '%s._201102_skel_saveDir' %
                         m_nodeName),
    ...
    def setAttribute(self, m_obj, m_attrName, m_attrValue):
        ...
        try:
            ...
            self.errorHandle.raiseMessage(
                "Can't unlock %s._201102_skel_skeletons attribute" %
                m_nodeName)
            return False
        # -----------------------------------------------------------------------------------------------
        ...
        if ("_201102_skel_saveDir" == m_attrName):
            ...
            try:
                ...
                self.errorHandle.raiseMessage(
                    "Can't setAttr %s._201102_skel_saveDir" %
                    m_nodeName)
        # -----------------------------------------------------------------------------------------------
        # _201102_skel_isSel
        # -----------------------------------------------------------------------------------------------
        if ("_201102_skel_isSel" == m_attrName):
            ...
            try:
                ...
                self.errorHandle.raiseMessage(
                    "Can't setAttr %s._201102_skel_isSel" %
                    m_nodeName)
        # ------------- lock --------------
        try:
            cmds.setAttr('%s._201102_skel_skeletons' % m_nodeName, lock=True)
            ...
            self.errorHandle.raiseMessage(
                "Can't lock %s._201102_skel_skeletons attribute" %
                m_nodeName)
        # ---------------------------------
        return False
    ...
    def iterItems(self):
        ...
        # ------------------------------------------------------------
        m_returnHash = {}
        # [ maxLen( getShortName( m_obj ) ), maxLen( m_obj ), maxLen( m_saveDir ) ]
        ...
        while not m_listIt.isDone():
            ...
            try:
                ...
                m_isSelected = cmds.getAttr(
                    '%s._201102_skel_isSel' % m_nodeName)
                m_saveDir = cmds.getAttr(
                    '%s._201102_skel_saveDir' %
                    m_nodeName)
                m_saveDirIsDefault = cmds.getAttr(
                    '%s._201102_skel_saveDirIsDefault' % m_nodeName)
                ...
                m_objFilename = "%s/%s.skeleton" % (
                    m_saveDir, m_nodeNamePartial)
                ...
                m_list.append(
                    addSpacesToString(
                        m_nodeNamePartial,
                        m_maxLenght[0] -
                        len(m_nodeNamePartial)))    # 'human'
                m_list.append(
                    addSpacesToString(
                        m_nodeName,
                        m_maxLenght[1] -
                        len(m_nodeName)))           # '|group1|human'
                # 'x:/resources/exported/animations/skeletons/human'
                m_list.append(
                    addSpacesToString(
                        m_objFilename,
                        m_maxLenght[2] -
                        len(m_objFilename)))
                # return string 'human    |group1|human
                # x:/resources/exported/animations/skeletons/human'
                m_returnHash[m_nodeName] = (
                    bool(m_isSelected), m_saveDir, string.join(
                        m_list, '    '), m_objFilenamePartial)
            except BaseException:
    ...
    def exportData(self, m_onlySelected=False):
        ...
        while not m_listIt.isDone():
            ...
            try:
                m_listIt.getDagPath(m_objPath)  # get current Dag
                m_nodeFn.setObject(m_objPath.node())  # set MFn to MObj
                m_nodeFn.attribute('_201102_skel_skeletons')  # '|group1|human'
                m_nodeFn.attribute('_201102_skel_isSel')  # True
                # 'XRAY_RESOURCES_PATH/animations/skeletons'
                m_nodeFn.attribute('_201102_skel_saveDir')
                m_nodeFn.attribute('_201102_skel_saveDirIsDefault')  # True
                ...
                m_objNameHash = m_saveDir.replace(
                    m_xrayResourcesPathSkeletons, '', 1) + '/' + m_nodeNamePartial   # '/human'
                # 'human    |group1|human    x:/resources/exported/animations/skeletons/human'
                m_objString = self.getObjLabelFromNode(m_obj)
            except BaseException:
                self.errorHandle.raiseMessage(
                    "m_skeletonNode.exportData %s item Error" % m_obj)
            else:
                # -------------------------------
