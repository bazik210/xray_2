...
def main():
    ...
    for m_option, m_value in opts:
        if m_option in ("-o", "--option"):
            m_stk2_root_path = m_value
        elif "--mesh" == m_option:
            m_options.append('mesh')
        elif "--skeleton" == m_option:
            m_options.append('skeleton')
        elif "--skin" == m_option:
            m_options.append('skin')
        elif "--anim" == m_option:
            m_options.append('anim')
        elif "--anim_add" == m_option:
            m_options.append('anim_add')
...
def processAllScenes(m_options):
    ...
    m_errorHandle.raiseMessage(
        time.strftime(
            "Exporter Message Log start in %d/%m/%Y %H:%M:%S",
            time.localtime()))
    m_mayaFiles = m_filesMaya()
    # get all maya files stored in the specified folder
    m_mayaFiles = m_mayaFiles.getAllMayaFiles(m_xrayResourcesPathModels)
    ...
    if (None is getRealXrayResourcesPath('models')):
        ...
        raise
    # Add Callback
    m_cb_id = OpenMaya.MSceneMessage.addCheckFileCallback(
        OpenMaya.MSceneMessage.kBeforeReferenceCheck,
        BeforeReferenceCheckCallback)
    ...
    if m_options.count("mesh"):
        m_xrayResourcesPathMeshesConfig = getRealXrayResourcesPath(
            'models') + '/categories.cfg'  # files with object tags
        ...
        if (os.path.exists(m_mayaFile.rstrip())):
            ...
            try:
                ...
                cmds.file(
                    ...
                    open=True,
                    type='mayaBinary',
                    force=True)  # open file as mayaBinary
                ...
                m_errorHandle.raiseMessage(
                    "ERROR Can't open file %s " % m_mayaFile)
                sys.stderr.write("ERROR Can't open file %s " % m_mayaFile)
                # m_erroNumber = 3
                # sys.exit( m_erroNumber )
                m_file_stack.append(m_mayaFile.rstrip())
        else:
            m_errorHandle.raiseMessage(
...
def doExportAnimCommon(m_node, m_errorHandle, m_mode, mayaFileName):
    ...
    if (m_node):
        if (m_node.getNode()):  # check for existing 'xray_anim_lister' node in the file
            try:
                if ("anim" == m_mode):
                    m_temp = m_node.exportDataIgnoreADD()  # export all non-additive animations
                if ("anim_add" == m_mode):
                    m_temp = m_node.exportDataALL()  # export all animations stored in the node
                if (m_temp):
                    m_erroNumber = m_temp
                    m_file_stack.append(mayaFileName)
