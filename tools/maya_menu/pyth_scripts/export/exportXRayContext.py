...
class m_meshTrace(object):
    ...
    def broadcastMessage(self):
        ...
        if (None is not self.msg):
            if (2 == OpenMaya.MGlobal_mayaState()):
                m_fileName = sys.path[len(sys.path) - 1]
                m_fileName = string.replace(
                    m_fileName, 'scripts', '') + 'xrayMayaLog.txt'
...
class m_animLayers(object):
    ...
    def getAnimLayers(self, m_node):
        ...
        try:
            if (cmds.animLayer(m_node, query=True, exists=True)):
                m_animLayers = cmds.animLayer(
                    m_node, query=True, children=True)
                for m_layer in m_animLayers:
                    m_animLayersTemp = cmds.animLayer(
                        m_layer, query=True, children=True)
                    if (None is m_animLayersTemp or 0 == len(m_animLayersTemp)):
                        self.m_obj_array.append(
                            getMObjectFromNodeName(m_layer))
                    else:
                        self.m_obj_array.append(
                            getMObjectFromNodeName(m_layer))
    ...
    def animLayersMuteSnapshot(self):
        ...
        for m_ind in range(self.m_obj_array.length()):
            self.m_animLayersMute.append(
                cmds.animLayer(
                    getNodeNameFromeMObject(
                        self.m_obj_array[m_ind]),
                    query=True,
                    mute=True))
        ...
        for m_ind in range(self.m_obj_array.length()):
            cmds.animLayer(
                getNodeNameFromeMObject(
                    self.m_obj_array[m_ind]),
                edit=True,
                mute=self.m_animLayersMute[m_ind])
...
class OrderedRawConfigParser(configparser.RawConfigParser):
    ...
    def __init__(self, defaults=None, dict_type=dict):
        configparser.RawConfigParser.__init__(
            self, defaults=None, dict_type=dict)
            ...
            for key in sorted(self._sections[section]):
                if key != "__name__":
                    fp.write(
                        "%s = %s\n" %
                        (key, str(
                            self._sections[section][key]).replace(
                            '\n', '\n\t')))
            fp.write("\n")
# -----------------------------------------------------------------------------------
# UTILITIES
...
def getFullPathToTag(m_tag=None):
    ...
    if (m_tag is None):
        ...
        return None
    for key in list(m_categoryTree.keys()):  # search in global array
        if (0 != m_categoryTree[key].count(m_tag)):
            return ("%s|%s" % (key[1], m_tag))
    errorHandle.raiseMessage(
        "getFullPathToTag Input tag %s don't exist in dictionary m_categoryTree" %
        m_tag)
    return None
...
def getRealXrayResourcesPath(m_path='models'):
    ...
    XRAY_RESOURCES_PATH = mel.eval('getenv "XRAY_RESOURCES_PATH" ')

    if ('models' == m_path):
        m_xrayResourcesPath = XRAY_RESOURCES_PATH + '/exported/models'
    elif ('animations/single' == m_path):
        m_xrayResourcesPath = XRAY_RESOURCES_PATH + '/exported/animations/single'
    elif ('skeletons' == m_path):
        m_xrayResourcesPath = XRAY_RESOURCES_PATH + '/exported/animations/skeletons'
    elif ('physics' == m_path):
        m_xrayResourcesPath = XRAY_RESOURCES_PATH + '/exported/physics'
    elif ('scenes' == m_path):
        m_xrayResourcesPath = XRAY_RESOURCES_PATH + '/sources/scenes'

    # --------------------------------
    if (os.path.exists(m_xrayResourcesPath)
        ):  # check if path to directory with resources exists
        return m_xrayResourcesPath  # initializaton done
    else:
        errorHandle.raiseMessage(
            'path [' +
            m_xrayResourcesPath +
            ']does not exist, check XRAY_RESOURCES_PATH variable in maya.env')
        return None
...
def getRealPathToReference(m_filepath):
    ...
    if (not os.path.exists(m_filepath)):
        m_filepathTmp1 = "%s%s" % (
            m_realPath, os.path.splitdrive(m_filepath)[1])
        if (not os.path.exists(m_filepathTmp1)):
            try:
                m_filepathTmp2 = "%s%s" % (m_realPath, string.split(
                    m_filepath, 'resources/sources/scenes')[1])
            except BaseException:
...
def addSpacesToString(m_inputStr, m_numberOfSpaces):
    ...
    """
    for n in range(m_numberOfSpaces):
        m_inputStr += ' '
    return m_inputStr
...
def isMeshHasSkinCluster(m_objName):
    ...
    try:
        ...
        return False
    m_dagIterator = OpenMaya.MItDag(
        OpenMaya.MItDag.kDepthFirst,
        OpenMaya.MFn.kInvalid)        # create NULL dagIterator
    m_dagIterator.reset(
        m_objPath.node(),
        OpenMaya.MItDag.kDepthFirst,
        OpenMaya.MFn.kInvalid)  # connect dagIterator with MDagPath
    while not m_dagIterator.isDone():  # iterate
        ...
        if (m_objCurr.hasFn(OpenMaya.MFn.kMesh)):  # if current == kMesh
            ...
            try:  # iterate in m_objNameMObj throw DependencyGraph based on kSkinClusterFilter
                m_itDG = OpenMaya.MItDependencyGraph(
                    m_objNameMObj,
