...
class m_meshNode(object):
        ...
        while not m_dagIterator.isDone():
            ...
            if (m_objCurr.hasFn(OpenMaya.MFn.kTransform)):
                ...
                if (not m_nodeFn.isFromReferencedFile()):
                    try:
                        m_nodeFn.attribute('_201102_objects')
                        m_nodeFn.attribute('_201102_isSel')
                        m_nodeFn.attribute('_201102_type')
                        m_nodeFn.attribute('_201102_selTags')
                        m_nodeFn.attribute('_201102_saveDir')
                        # print( "%s %s" %( m_objPath.fullPathName(), m_nodeFn.name() ) )
                        m_selectionList.add(m_objPath)
                    except BaseException:
                        pass
            next(m_dagIterator)  # next item
    ...
    def addObj(
        ...
        try:
            ...
            self.errorHandle.raiseMessage(
                "Can't set MObject() to MFnDependencyNode()")
            return False

        try:
            m_nodeFn.attribute('_201102_objects')
            self.errorHandle.raiseMessage(
                "Attribute %s._201102_objects already exists" %
                m_nodeFn.name())
            return False
        except BaseException:
            pass
        ...
        try:
            ...
            cmds.addAttr(
                m_nodeName,
                parent='_201102_objects',
                longName='_201102_isSel',
                niceName='Is Selected',
                attributeType='bool')
            cmds.addAttr(
                m_nodeName,
                parent='_201102_objects',
                longName='_201102_type',
                niceName='Type',
                dataType='string')
            cmds.addAttr(
                m_nodeName,
                parent='_201102_objects',
                longName='_201102_selTags',
                niceName='Tags',
            ...
            self.errorHandle.raiseMessage(
                "Can't add attribute '_201102_objects' to node %s" %
                m_nodeName)
            return False
        ...
        cmds.setAttr('%s._201102_selTags' %
                     m_nodeName, type='stringArray', *
                     ([len(m_objSelTags)] +
                      m_objSelTags))
        cmds.setAttr(
            '%s._201102_saveDir' %
    ...
    def delObj(self, m_obj):
        ...
        try:
            ...
            self.errorHandle.raiseMessage(
                "Can't find %s._201102_objects attribute" %
                m_nodeName)
        return False
    ...
    def printObj(self):
        ...
        while not m_listIt.isDone():
            ...
            try:
                ...
                m_nodeFn.attribute('_201102_saveDir')
                self.errorHandle.raiseMessage(
                    "KEY: %s <==> SEL: %s TYPE: %s TAG: %s DIR: %s" %
                    (m_nodeName,
                     cmds.getAttr(
                         '%s._201102_isSel' %
                         m_nodeName),
        ...
        while not m_listIt.isDone():
            ...
            if (m_lod0MDagPath):
                ...
                try:
                    ...
                    self.errorHandle.raiseMessage(
                        "Can't parent %s to %s" %
                        (m_visNodeMDagPath.fullPathName(), m_nodeName))
                    return False
                try:
                    cmds.rename(m_visNodeMDagPath.fullPathName(), 'vis')
                except BaseException:
                    self.errorHandle.raiseMessage(
                        "Can't rename %s to vis" %
                        m_visNodeMDagPath.fullPathName())
                    return False
                try:
                    cmds.parent(
                        m_lod0MDagPath.fullPathName(),
                        m_visNodeMDagPath.fullPathName())
                except BaseException:
                    self.errorHandle.raiseMessage(
                        "Can't parent %s to %s" %
                        (m_lod0MDagPath.fullPathName(), m_visNodeMDagPath.fullPathName()))
                    return False
            ...
            if (m_clod0MDagPath):
                ...
                try:
                    ...
                    self.errorHandle.raiseMessage(
                        "Can't parent %s to %s" %
                        (m_colNodeMDagPath.fullPathName(), m_nodeName))
                    return False
                ...
                try:
                    ...
                    self.errorHandle.raiseMessage(
                        "Can't rename %s to col" %
                        m_colNodeMDagPath.fullPathName())
                    return False

                try:
                    cmds.parent(
                        m_clod0MDagPath.fullPathName(),
                        m_colNodeMDagPath.fullPathName())
                except BaseException:
                    self.errorHandle.raiseMessage(
                        "Can't parent %s to %s" %
                        (m_clod0MDagPath.fullPathName(), m_colNodeMDagPath.fullPathName()))
                    return False
    ...
    def setObjTag(self, m_obj, m_objSelTags):
        ...
        try:
            ...
            self.errorHandle.raiseMessage(
                "Can't setAttr %s._201102_selTags" %
                m_nodeName)
        return False
    ...
    def setObjType(self, m_obj, m_objType):
        ...
        if (0 == m_objectType.count(m_objType)):
            ...
            self.errorHandle.raiseMessage(
                "m_meshNode::setObjType Entered type %s don't specified in global array m_objectType" %
                m_objType)
            return False
        m_objPath = getMDagPathFromNodeName(m_obj)
    ...
    def setSaveDir(self, m_obj, m_saveDir):
        ...
        try:
            ...
            cmds.setAttr(
                '%s._201102_saveDir' %
                m_nodeName, m_saveDir, type='string')
            # ------------- lock --------------
            cmds.setAttr('%s._201102_objects' % m_nodeName, lock=True)
            cmds.setAttr('%s._201102_saveDir' % m_nodeName, lock=True)
            return True
            ...
            self.errorHandle.raiseMessage(
                "Can't setAttr %s._201102_saveDir" %
                m_nodeName)
        return False
    ...
    def setAllSelectionStates(self, m_IsSelected=False):
        ...
        while not m_listIt.isDone():
            ...
            try:
                m_nodeFn.aAttribute('_201102_objects')
                m_nodeFn.attribute('_201102_isSel')
                # ----------- unlock --------------
                cmds.setAttr('%s._201102_objects' % m_nodeName, lock=False)
                cmds.setAttr('%s._201102_isSel' % m_nodeName, lock=False)
    ...
    def iterItems(self):
        ...
        # ------------------------------------------------------------
        m_returnHash = {}
        # [maxLen(getShortName(m_obj)), maxLen(m_obj), maxLen(m_objType), maxLen(m_saveDir)]
            ...
            try:
                ...
                m_list.append(
                    addSpacesToString(
                        m_nodeNamePartial,
                        m_maxLenght[0] -
                        len(m_nodeNamePartial)))    # box_01
                m_list.append(
                    addSpacesToString(
                        m_nodeName,
                        m_maxLenght[1] -
                        len(m_nodeName)))           # |props|box|box_01
                m_list.append(
                    addSpacesToString(
                        m_objType,
                        m_maxLenght[2] -
                        len(m_objType)))            # Static
                m_list.append(
                    addSpacesToString(
                        m_saveDir,
                        m_maxLenght[3] -
                ...
                if (m_objInput == m_nodeName):
                    ...
                    m_list.append(
                        addSpacesToString(
                            ...
                            m_maxLenght[3] -
                            len(m_saveDir)))            # props
                    # return string 'box_01    |props|box|box_01    Static
                    # props'
    ...
    def exportData(self, m_onlySelected=False, m_ndb=False):
        ...
        #     READ CONFIG
        # -------------------------
        m_xrayResourcesPathMeshesConfig = getRealXrayResourcesPath(
            'models') + '/categories.cfg'
        if (os.path.exists(m_xrayResourcesPathMeshesConfig)):
            m_configRead = configparser.ConfigParser()
            try:
                # check file for parsing errors
                m_configRead.read(m_xrayResourcesPathMeshesConfig)
            except configparser.ParsingError:  # File contains parsing errors
                self.errorHandle.raiseMessage(
                    "m_meshNode::exportData categories.cfg file contains parsing errors")
            else:
                try:  # Don't have parsing errors
                    m_configReadSectionStatic = m_configRead.items(
                        'Static')  # try to read 'static' section
                    m_configReadSectionDynamic = m_configRead.items(
                        'Dynamic')  # try to read 'dynamic' section
                except configparser.NoSectionError:  # section don't exists
                    self.errorHandle.raiseMessage(
                        "m_meshNode::exportData categories.cfg section don't exists")
        ...
        while not m_listIt.isDone():
            ...
            try:
                m_listIt.getDagPath(m_objPath)  # get current Dag
                m_nodeFn.setObject(m_objPath.node())  # set MFn to MObj
                m_nodeFn.attribute('_201102_objects')    # '|boxes|box1'
                m_nodeFn.attribute('_201102_isSel')      # True
                m_nodeFn.attribute('_201102_type')       # 'Static'
                # ['Location|Industrial', 'Architectural|Building']
                m_nodeFn.attribute('_201102_selTags')
                # 'XRAY_RESOURCES_PATH/models/props/boxes'
                m_nodeFn.attribute('_201102_saveDir')
                ...
                m_objSelTags = cmds.getAttr('%s._201102_selTags' % m_nodeName)
                m_saveDir = cmds.getAttr('%s._201102_saveDir' % m_nodeName)
                m_objNameHash = m_saveDir.replace(
                    m_xrayResourcesPathMeshes,
                    '',
                    1) + '/' + m_nodeNamePartial   # 'props/boxes/box1'
                # 'box_01    |props|box|box_01    Static    props/boxes'
                m_objString = self.getObjLabelFromNode(m_obj)
            except BaseException:
                self.errorHandle.raiseMessage(
                    "m_meshNode.exportData %s item Error" % m_obj)
            else:
                # -------------------------------
                if (not m_onlySelected):  # reload all objects
                    m_IsSelected = True
                if (bool(m_IsSelected)):
                    if m_objNameHash in m_storedDataStatic:
                        del m_storedDataStatic[m_objNameHash]
                    if m_objNameHash in m_storedDataDynamic:
                    ...
                    if (m_objType == 'Static'):
                        ...
                        m_objLOD0filename = getRealXrayResourcesPath(
                            'models') + m_objNameHash
                        m_objLOD0filename = m_objLOD0filename.lower()
                        # -------------------------------------------------------------------------------------------------------------------------
                        if (cmds.objExists(m_objLOD0visual)):
                            try:
                                cmds.xray_solid_visual_export(
                                    ndb=m_ndb,
                                    ...
                                    thq=True)
                                # redefine tags
                                m_storedDataStatic[m_objNameHash] = m_objSelTags
                        ...
                        # None LODs
                            ...
                            self.errorHandle.raiseMessage(
                                "ERROR   %s  == > Can't find %s" %
                                (m_objString, m_objLOD0visual))
                            m_erroNumber = 4
                    if (m_objType == 'Dynamic'):
                        # m_storedDataDynamic[m_objNameHash] = m_objSelTags #
                        # redefine tags
                        self.errorHandle.raiseMessage(
                            "DYNAMIC EXPORTER  %s  == > DON'T WORK YET" % m_objString)
                        # -----------------------
        ...
        for m_key in list(m_storedDataStatic.keys()):
            m_configWrite.set(
                'Static', m_key, string.join(
                    m_storedDataStatic[m_key], ' '))
        for m_key in list(m_storedDataDynamic.keys()):
            m_configWrite.set(
                'Dynamic', m_key, string.join(
                    m_storedDataDynamic[m_key], ' '))
