    ...
    for m_mat in m_listOfMaterials:
        if (not isObjFromReferencedFile(m_mat)):
            if (changeMaterialType(m_mat, m_destType)):
                sys.stdout.write(
                    "%s successfully changed FROM %s TO %s\n" %
...
def changeMaterialType(m_oldMatName, m_newMatType):
    ...
    if (not m_matTypes.count(m_newMatType)):
        if (m_debug):
            sys.stderr.write("Specify material type, like: lambert\n")
        return False
    try:
        m_newMatName = cmds.shadingNode(m_newMatType, asShader=True)
        if (m_debug):
            sys.stdout.write(
                "new material %s successfully created\n" %
                m_newMatName)
    except BaseException:
        if (m_debug):
            sys.stderr.write("Can't create new material\n")
        return False
    ...
    for m_oldSG in m_oldSGNames:
        try:
            ...
            if (m_debug):
                sys.stdout.write(
                    "Successful connectAttr from %s.outColor to %s.surfaceShader\n" %
                    (m_newMatName, m_oldSG))
        except BaseException:
            if (m_debug):
                sys.stderr.write(
                    "Can't connectAttr from %s.outColor to %s.surfaceShader\n" %
                    (m_newMatName, m_oldSG))
            return False
    try:
        cmds.delete(m_oldMatName)
        if (m_debug):
            sys.stdout.write("%s successfully deleted\n" % m_oldMatName)
    except BaseException:
        if (m_debug):
...
def copyAllShaderParameters(m_oldMatName, m_newMatName):
    ...
    for m_attr in m_commonShaderAttributes:
        ...
        try:
            ...
            if (m_debug):
                sys.stdout.write(
                    "Successful setAttr %s.%s to %s\n" %
                    (m_newMatName, m_attr, m_oldAttr))
        except BaseException:
            if (m_debug):
                sys.stderr.write(
                    "Can't setAttr %s.%s to %s\n" %
                    (m_newMatName, m_attr, m_oldAttr))
...
def copyAllShaderConnections(m_oldMatName, m_newMatName):
    ...
    for m_attr in m_commonShaderConnections:
        m_oldConnection = cmds.connectionInfo(
            "%s.%s" %
            (m_oldMatName, m_attr), sourceFromDestination=True)
        if (m_oldConnection):
            try:
                cmds.connectAttr(
                    m_oldConnection, "%s.%s" %
                ...
                if (m_debug):
                    sys.stdout.write(
                        "Successful connectAttr from %s to %s.%s \n" %
                        (m_oldConnection, m_newMatName, m_attr))
            except BaseException:
                if (m_debug):
                    sys.stderr.write(
                        "Can't connectAttr from %s to %s.%s\n" %
                        (m_oldConnection, m_newMatName, m_attr))
...
def getSGFromMaterial(m_material):
    ...
    if ("shader/surface" == m_classifications):
        if (cmds.connectionInfo("%s.outColor" % m_material, isSource=True)):
            m_destSGs = cmds.connectionInfo(
                "%s.outColor" %
                m_material, destinationFromSource=True)
