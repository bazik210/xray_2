...
def renamePlace2dTextureNodes():
    ...
    for m_place2dTexture in m_listPlace2dTexture:
        if (not isObjFromReferencedFile(m_place2dTexture)):
            m_connectedNodes = cmds.listConnections(
                m_place2dTexture, skipConversionNodes=True)
            if (m_connectedNodes):
                for m_node in m_connectedNodes:
                    if ('file' == cmds.nodeType(m_node)):
                        try:
                            m_file = cmds.getAttr(
                                "%s.fileTextureName" % m_node)
                            ...
                            try:
                                ...
                                print((
                                    "Can't rename %s to %s_place2dTexture" %
                                    (m_place2dTexture, m_file)))
                            else:
                                print((
                                    "%s to %s_place2dTexture Renamed Successfully!" %
                                    (m_place2dTexture, m_file)))
                        break
    return m_result
                    ...
                    if ('file' == cmds.nodeType(m_node)):
                        try:
                            ...
                            try:
                                ...
                                print((
                                    "Can't rename %s to %s_MatInfo" %
                                    (m_mi, m_file)))
                            else:
                                print((
                                    "%s to %s_MatInfo Renamed Successfully!" %
                                    (m_mi, m_file)))
    return m_result
                    ...
                    if ('file' == cmds.nodeType(m_node)):
                        try:
                            ...
                            try:
                                ...
                                print((
                                    "Can't rename %s to %s_s" %
                                    (m_sg, m_file)))
                            else:
                                print((
                                    "%s to %s_s Renamed Successfully!" %
                                    (m_sg, m_file)))
    return m_result
...
def renameBump2DNodes():
    ...
    for m_bump2d in m_bump2dList:
        if (not isObjFromReferencedFile(m_bump2d)):
            m_connectedNodes = cmds.listConnections(
                m_bump2d, destination=False, source=True)
            if (m_connectedNodes):
                for m_node in m_connectedNodes:
                    if ('file' == cmds.nodeType(m_node)):
                        try:
                            ...
                            try:
                                ...
                                print((
                                    "Can't rename %s to %s_bump2d" %
                                    (m_bump2d, m_file)))
                            else:
                                print((
                                    "%s to %s_bump2d Renamed Successfully!" %
                                    (m_bump2d, m_file)))
    return m_result
        ...
        if (not isObjFromReferencedFile(m_fileNode)):
            try:
                ...
                try:
                    ...
                    print((
                        "Can't rename %s to %s_%s" %
                        (m_fileNode, m_fileName, m_fileExt)))
                else:
                    print((
                        "%s to %s_%s Renamed Successfully!" %
                        (m_fileNode, m_fileName, m_fileExt)))
    return m_result
...
def getFirstFileNode(m_nodeInput):
    m_connectedNodes = cmds.listConnections(
        m_nodeInput, destination=False, source=True)
    if (m_connectedNodes):
        for m_node in m_connectedNodes:
            if ('file' == cmds.nodeType(m_node)):
                return m_node
...
def getMaterialFromSG(m_sg):
    m_material = None
    if ("shadingEngine" == cmds.nodeType(m_sg) and cmds.connectionInfo(
            "%s.surfaceShader" % m_sg, isDestination=True)):
        m_material = cmds.connectionInfo(
            "%s.surfaceShader" %
            m_sg, sourceFromDestination=True)
