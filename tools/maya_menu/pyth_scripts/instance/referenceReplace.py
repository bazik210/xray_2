...
class m_replaceRefWin(object):
    ...
    def __init__(self):
        ...
        self.m_window = cmds.window(
            m_replaceReferencesGuiID,
            title="Replace Reference",
            width=300,
            height=200)
        self.m_formLayout = cmds.formLayout(parent=self.m_window)
        # ----------------------------------------------------------------------
        self.m_textLoadAsset = cmds.text(
            parent=self.m_formLayout,
            label='Load a ASSET from referenced file to use as a SOURCE for replacing',
            align='left')
        self.m_textFieldAssetName = cmds.textFieldButtonGrp(
            parent=self.m_formLayout,
            label='Asset:',
            editable=False,
            ...
            columnWidth3=(
                ...
                220,
                80),
            buttonCommand=lambda *args: self.uiCallback(
        ...
        self.m_separator1 = cmds.separator(parent=self.m_formLayout,
                                           height=5, style='in')
        self.m_textSelectByMask = cmds.text(
            parent=self. m_formLayout,
            label='Select objects by mask',
            align='left')
        self.m_textFieldFilter = cmds.textFieldButtonGrp(
            parent=self.m_formLayout,
            label='Filter:',
            editable=True,
            buttonLabel='Select',
        ...
        self.m_checkBoxAllowInstances = cmds.checkBox(
            parent=self.m_formLayout,
            label='Allow Instances',
            value=False,
            changeCommand=lambda *args: self.uiCallback(
                "m_checkBoxAllowInstances",
                args),
            annotation="Allow Instances when press 'Select' button")
        self.m_checkBoxIgnoreTrWithoutShapes = cmds.checkBox(
            parent=self.m_formLayout,
            label='Ignore Transforms without shapes',
            value=True,
            changeCommand=lambda *args: self.uiCallback(
                "m_checkBoxIgnoreTrWithoutShapes",
                args),
        ...
        self.m_separator2 = cmds.separator(parent=self.m_formLayout,
                                           height=5, style='in')
        self.m_textReplaceSelection = cmds.text(
            parent=self.m_formLayout,
            label='Replace SELECTION to SOURCE',
            align='left')
        self.m_buttonReplace = cmds.button(
            parent=self.m_formLayout,
            label='Replace',
            width=80,
            command=lambda *args: self.uiCallback(
                "m_buttonReplace",
                args),
        ...
        if ('m_textFieldAssetNameBtn' == m_input):
            ...
            if (m_temp):
                ...
                cmds.textFieldButtonGrp(
                    self.m_textFieldAssetName,
                    edit=True,
                    fileName=self.m_dagPathSelectedAsset.fullPathName())
        # -------------------------------
        #    m_textFieldFilterBtn
        # -------------------------------
        if ('m_textFieldFilterBtn' == m_input):
            m_maskStr = cmds.textFieldButtonGrp(
                self.m_textFieldFilter, query=True, fileName=True)
            m_allowInstances = cmds.checkBox(
                self.m_checkBoxAllowInstances, query=True, value=True)
            m_ignoreTrWithoutShapes = cmds.checkBox(
                self.m_checkBoxIgnoreTrWithoutShapes, query=True, value=True)
            selectObjectsByMask(
                m_maskStr,
                m_ignoreTrWithoutShapes,
                not m_allowInstances)
        # -------------------------------
        #    m_buttonReplace
        # -------------------------------
    ...
    if (m_selList):
        ...
        if (not isObjFromReferencedFile(m_selObj)):
            OpenMaya.MGlobal.displayWarning(
                "%s not from Referenced file" % m_selObj)
            return None
...
def replaceObjToRef(m_inputObjStr):
    ...
    while not m_selListIt.isDone():
        ...
        if (m_objCurr.hasFn(OpenMaya.MFn.kTransform)):
            ...
            # ignore referenced
            if (not m_fnDagCurr.isFromReferencedFile()
                    and not isObjInDagContainer(m_fnDagCurr.fullPathName())):
                # ------------------------------------------
                m_filename = cmds.reference(
                    m_inputObjStr, query=True, filename=True)
                ...
                TRSPPSPfromTo(
                    ...
                    m_dagPathLastAdded.fullPathName())
                if (not m_objParent.hasFn(OpenMaya.MFn.kWorld)):
                    ...
                    try:
                        cmds.parent(
                            m_dagPathLastAdded.fullPathName(),
                            m_fnDagNodeParent.fullPathName())
