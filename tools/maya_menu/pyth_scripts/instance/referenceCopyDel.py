...
def SelectAssetsBySelection():
    ...
    while not m_dagIterator.isDone():
        ...
        if (m_dagCurr.isFromReferencedFile()
                and m_objCurr.hasFn(OpenMaya.MFn.kDagContainer)):
            m_filenameCurr = cmds.reference(
...
def goTroughtTree(
        m_objStrChild,
        m_objStrParent='world',
        m_objStrNewParent='world'):
