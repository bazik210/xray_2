...
def transformToInstance(m_stat=False):
    ...
    # ------------------------------------------
    m_selectionList = OpenMaya.MSelectionList()
    # selection list for instanced objects
    ...
    while not m_selListIt.isDone():
        ...
        try:
            ...
            if (m_dagPath.isInstanced()):  # object already instanced
                # get all instances
                m_allParents = cmds.listRelatives(
                    m_dagPath.fullPathName(), fullPath=True, allParents=True)
                for m_dagString in m_allParents:
                    m_selectionListFinal.add(m_dagString)
            else:
                m_dagPath = OpenMaya.MDagPath()
                m_selListIt.getDagPath(m_dagPath)
                m_selectionListFinal.add(
                    m_dagPath.fullPathName())  # fullname without Shape
        next(m_selListIt)
    # ---------------------- create iterator ------------------------------
    if (m_stat):
    ...
    while not m_finListIt.isDone():
        ...
        try:
            ...
            print(("Can't rename %s to %s " %
                  (m_tmpObjDagNode.fullPathName(), m_resultName)))
        m_tmpObjMObject = getMObjectFromNodeName(
            m_tmpObjDagNode.fullPathName())
        if (not m_tmpObjMObject):
            print((
                "Can't get MObject from name %s" %
                m_tmpObjDagNode.fullPathName()))
            break
        try:
            ...
            print((
                "Can't connect MFnTransform with MObject %s " %
                m_tmpObjDagNode.fullPathName()))
            break
        m_tmpObjParentDest = getParentDagNodeFromNodeName(
            m_dagPath.fullPathName())
        m_tmpObjParentSrc = getParentDagNodeFromNodeName(
            m_tmpObjDagNode.fullPathName())
        if (not m_tmpObjParentDest):
            print((
                "Can't get parent DagNode from name %s" %
                m_dagPath.fullPathName()))
            break
        ...
        if (m_tmpObjParentDest.isDefaultNode()):  # parent to world
            ...
            try:
                cmds.parent(
                    m_tmpObjDagNode.fullPathName(),
                    m_tmpObjParentDest.fullPathName())
            except BaseException:
                print((
                    "Can't parent %s to %s " %
                    (m_tmpObjDagNode.fullPathName(),
    ...
    if (0 != m_selectionList.length()):
        try:
            ...
            try:
                ...
                print((
                    "Can't rename %s to %s " %
                    (m_dagPath.fullPathName(), m_inputStr)))
