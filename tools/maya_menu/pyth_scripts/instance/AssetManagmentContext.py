...
def selectObjectsByMask(
        m_maskStr,
        m_ignoreTransformsWithoutShape=True,
        m_ignoreInstances=False):
    """
    ...
    m_filterUnion = cmds.itemFilter(
        intersect=(m_filterTransforms, m_filterMask))
    # get list by Filter
        ...
        if (m_selList):
            for m_obj in m_selList:
                ...
                for m_shape in m_shapeList:
                    m_instList = cmds.listRelatives(
                        m_shape, fullPath=True, allParents=True)
...
def TRSPPSPfromTo(m_source, m_destination):
    ...
    """
    try:
        m_translation = cmds.xform(
            m_source,
            query=True,
            worldSpace=True,
            translation=True)
        m_rotation = cmds.xform(
            m_source,
            query=True,
            worldSpace=True,
            rotation=True)
        # Warning: Cannot query scale in worldspace.  Defaulting to local
        # space.
        m_scale = cmds.xform(m_source, query=True, relative=True, scale=True)
        m_rotatePivot = cmds.xform(
            m_source,
            query=True,
            worldSpace=True,
            rotatePivot=True)
        m_scalePivot = cmds.xform(
            m_source,
            query=True,
            worldSpace=True,
            scalePivot=True)
...
def isObjInDagContainer(m_objName, m_result=False):
    ...
    if (not m_objParent.hasFn(OpenMaya.MFn.kWorld)):
        m_result = isObjInDagContainer(
            m_fnDagNodeParent.fullPathName(), m_result)
    # print "obj = %s     parent = %s     world = %s    res = %s" %(
    # m_objName, m_fnDagNodeParent.fullPathName() , m_objParent.hasFn(
    # OpenMaya.MFn.kWorld ), m_result )
    return m_result
    ...
    if (not os.path.exists(m_createDir)):
        try:
            ...
            OpenMaya.MGlobal.displayWarning(
                "Can't create directory %s" % m_createDir)
            return False
    return m_createDir
