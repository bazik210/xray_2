        ...
        if (0 != m_selectionList.length()):
            ...
            while not m_selListIt.isDone():
                ...
                m_dagIterator = OpenMaya.MItDag(
                    OpenMaya.MItDag.kDepthFirst, OpenMaya.MFn.kInvalid)
                m_dagIterator.reset(
                    m_dagPath.node(),
                ...
                while not m_dagIterator.isDone():
                    ...
                    next(m_dagIterator)
                next(m_selListIt)
            # ----------------- remove duplicates from  m_selectionListFinal an
            ...
            if (m_instDir):
                m_dagContainerIt = OpenMaya.MItSelectionList(
                    m_dagContainerList)
                while not m_dagContainerIt.isDone():
                    m_dagPathCurr = OpenMaya.MDagPath()
                    m_dagContainerIt.getDagPath(m_dagPathCurr)
                    # ----------------------------------------------------------------------------------
                    if (not isObjFromReferencedFile(m_dagPathCurr.fullPathName())
                            and isObjDagContainer(m_dagPathCurr.fullPathName())):
                        # select current Asset
                        cmds.select(m_dagPathCurr.fullPathName(), replace=True)
                        # create filename
                        m_filename = "%s/%s.mb" % (m_instDir,
                                                   getShortName(
                        ...
                        cmds.setAttr(
                            "%s.blackBox" %
                            m_dagPathCurr.fullPathName(), True)
                        try:
                            ...
                            OpenMaya.MGlobal.displayWarning(
                                "Could not save file %s" % m_filename)
                        else:
                            OpenMaya.MGlobal.displayInfo(
                                "Successful %s" % m_filename)
                        cmds.setAttr(
                            "%s.blackBox" %
                            m_dagPathCurr.fullPathName(), False)
                        m_tr.transfromsRestore()
                        # ----------------------------------------------------------------------------------
                    next(m_dagContainerIt)
...
class m_transformHideRestore(object):
    ...
    def setAsset(self, m_assetName):
        ...
        try:
            ...
            m_selectionList.add(m_assetName)
        except BaseException:
            return False
        try:
    ...
    def fixShadigGroup(self):
        ...
        while not m_dagIterator.isDone():
            ...
            if (m_objCurr.hasFn(OpenMaya.MFn.kMesh)):
                cleanPerFaceAssignment.cleanPerFaceAssignmentItemEasy(
    ...
    def getTransforms(self):
        ...
        if (not self.m_mobjLOD0.isNull()):
            ...
            while not m_dagIterator.isDone():
                ...
                m_dagCurr = OpenMaya.MFnDagNode(m_objCurr)
                if (m_objCurr.hasFn(OpenMaya.MFn.kTransform)):
                    if (not m_dagCurr.hasChild(self.m_mobjLOD0)
                            and self.m_mobjLOD0 != m_objCurr):
    ...
    def transfromsHide(self):
        ...
        if (m_result):
            for m_ind in range(self.m_obj_array.length()):
                ...
                cmds.setAttr(
                    "%s.visibility" %
                    m_dagCurr.fullPathName(), lock=True)
    ...
    def transfromsRestore(self):
        ...
        if (m_result):
            ...
            print("    >>> %s_LOD0 don't exists in Asset " %
                  getShortName(m_dagCurr.fullPathName()))
        # PARENT current Asset to old Parent
        if (not self.m_mobjParent.hasFn(OpenMaya.MFn.kWorld)):
            m_fnDagAsset = OpenMaya.MFnDagNode(self.m_mobjAsset)
            m_fnDagParent = OpenMaya.MFnDagNode(self.m_mobjParent)
            cmds.parent(
                m_fnDagAsset.fullPathName(),
                m_fnDagParent.fullPathName())
