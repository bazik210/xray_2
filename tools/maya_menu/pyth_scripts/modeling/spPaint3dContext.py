...
class intersectionPoint (object):
    ...
    def __init__(self, hitPoint, hitFace, hitTriangle, dagMesh):
        ...
        # MDagPath where the intersection occured (used to avoid having to
        # compute all normals on soon-to-be discarded intersections
        self.dagMeshTargetSurface = dagMesh
        # used to track the creation of an object at that intersectionPoint
        # (later used in the strokePointList class)
        self.timestamp = None
        # used to store the DAG path of the created geometry if it's actually a
        # valid intersection
    ...
    def doCrossProduct(self, v1, v2):
        ...
        '''
        vectNorm = om.MVector(((v1.y * v2.z) - (v1.z * v2.y)),
                              ((v1.z * v2.x) - (v1.x * v2.z)),
                              ((v1.x * v2.y) - (v1.y * v2.x)))
        vectNorm.normalize()
        return vectNorm
...
class intersectionList (object):
    ...
    def addPoint(self, ipoint):
        ...
        if (sp3d_log):
            print((
                "adding a new intersection to intersectionList (length of list: %i)" % len(
                    self.intersectionList)))
        # self.printList();
    ...
    def printList(self):
        ...
        for obj in self.intersectionList:
            print(("object created: %s (using source: %s)" %
...
class paintContext (object):
    ...
    def onPress(self):
        ...
        if (intersected):
            ...
            if (sp3d_dbg):
                logDebugInfo(
                    'finished creating object from the dag, appending to intersection list')
        ...
        if (intersected):
            ...
            if (len(self.strokeIntersectionList.intersectionList) == 0):
                ...
                if (self.uiValues.paintFlux):
                    ...
                    if (sp3d_log):
                        print(('intersection at X: %f | Y: %f | Z: %f |||| distance from previous: %f (x: %f | y: %f | z: %f)(threshold: %f)(length of list: %i)' % (intersected.hitPoint.x,
                                                                                                                                                                     intersected.hitPoint.y,
                                                                                                                                                                     intersected.hitPoint.z,
                    ...
                    # paintFlux set on timer
                    if (mc.timerX(
                            startTime=self.strokeIntersectionList.intersectionList[-1].timestamp) < self.uiValues.paintTimer):
    ...
    def createObject(self, intersection):
        ...
        if (self.uiValues.instance):
            ...
            # duplicating with preserve input connections and stuff docked onto
            # the transform and not the shape
            if (sp3d_dbg):
        ...
        if (self.uiValues.align):
            ...
            rx, ry, rz = getEulerRotationQuaternion(
                self.worldUp, intersection.getHitNormal(
                    self.uiValues.smoothNormal))
    ...
    def getState(self):
        ...
        if (modifiers & 1) > 0:
            if (self.shiftReleased):
                # shift was released in the previous iteration, this is a
                # keypress
                self.shiftReleased = False
                shift = True
        else:
            # shift is not currently pressed, reseting the tracking self
            # variable to released state
            self.shiftReleased = True

        # CTRL EVENT
        if (modifiers & 4) > 0:
            if (self.ctrlReleased):
                # ctrl was released in the previous iteration, this is a
                # keypress
                self.ctrlReleased = False
                ctrl = True
        else:
            # ctrl is not currently pressed, reseting the tracking self
            # variable to released state
            self.ctrlReleased = True
        ...
        if (modifiers & 8) > 0:
            if (self.altReleased):
                # alt was released in the previous iteration, this is a
                # keypress
                self.altReleased = False
                alt = True
        else:
            # alt is not currently pressed, reseting the tracking self variable
            # to released state
            self.altReleased = True
...
class placeCursor (object):
    ...
    def setCursorDAG(self, sourcedag, cursordag, deleteprevious=False):
        ...
        if (deleteprevious):
            if (mc.objExists(self.cursorDAG)):
                # deleting the previous cursor object and its parent group if
                # it's empty
    ...
    def align(self, rx=None, ry=None, rz=None):
        ...
        if ((rx is not None) and (ry is not None) and (rz is not None)):
            ...
            # realigning cursor with previously stored rotate tuple if it
            # exists
            if (self.cursorAlign):
...
class placeContext (object):
    ...
    def fetchCursorObject(self):
        ...
        # duplicating input connections on the parent transform is not carried
        # over if using the shape
    ...
    def fetchCursorTransform(self):
        ...
        if (self.uiValues.transformScale):
            cursorScale = self.transform.getRandomScale(
                self.uiValues.transformScaleUniform)
        else:
            # transform scale off
            cursorScale = (1, 1, 1)
    ...
    def ctrlEvent(self):
        ...
        self.cursor.setCursorDAG(newSourceDAG, newCursorDAG, True)
        # defaulting without parameters to position the new cursor to the
    ...
    def onBeforePress(self):
        ...
        self.cursor.transform(
            self.uiValues.transformRotate,
            self.uiValues.transformScale)
        # self.cursor.asTemplate() #issues with place and object hierarchies,
        # breaking the method for some reason
    ...
    def onPress(self):
        ...
        if (intersected):
            ...
            if (self.uiValues.align):
                rx, ry, rz = getEulerRotationQuaternion(
                    self.worldUp, intersected.getHitNormal(
                        self.uiValues.smoothNormal))
    ...
    def onRelease(self):
        ...
        '''
        # reverting template
        # self.cursor.asTemplate(False) #issues with object hierarchies for
        # some reason
...
def moveTo(dag, pos):
    ...
    mc.xform(dag,
             t=((transform[0] - scalePivot[0]) + pos.x,
...
def getEulerRotationQuaternion(upvector, directionvector):
    ...
    return math.degrees(
        quatAsEuler.x), math.degrees(
        quatAsEuler.y), math.degrees(
            quatAsEuler.z)
...
def getDAGObject(dagstring):
    ...
    meshDP = om.MDagPath()
    # sList.clear() #making sure to clear the content of the MSelectionList in
    # case we are looping through multiple objects
