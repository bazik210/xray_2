...
def printObjectSGInfo():
    ...
    while not m_listIt.isDone():
        # Get the DAG path of each element and extend the path to its shape
        # node
...
def cleanPerFaceAssignmentItem(m_path, m_bNeedReconectSG=False):
    ...
    # storoes the number of faces included in each shading group
    m_sameSGFaceCount = OpenMaya.MIntArray()
    # stores names of faces included in each shading group
    m_memberFaceNames = []
            ...
            while not m_tempFaceIt.isDone():
                if (-1 == m_lastIndices[0]):
                    ...
                    if (m_currentIndex > m_lastIndices[1] + 1):
                        m_aMemberFaceName += '%s.f[%d:%d] ' % (
                            m_path.fullPathName(), m_lastIndices[0], m_lastIndices[1])
...
def cleanPerFaceAssignmentItemEasy(m_node_name):
    ...
    # -----------------------------
    m_path.extendToShape()
    m_SGNames = cmds.listConnections(
        m_path.fullPathName(),
        destination=True,
        source=False,
        plugs=False,
    ...
    if (m_SGNames):
        for m_SG in m_SGNames:
            ...
            while not m_tempFaceIt.isDone():
                if (cmds.sets("%s.f[%s]" % (
                        m_path.fullPathName(), m_tempFaceIt.index()), isMember=m_SG)):
