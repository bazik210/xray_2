...
def rayIntersect(mesh, point, direction):
    ...
    # clear selection as may cause problem if the function is called multiple
    # times in succession
    om.MGlobal.clearSelectionList()
    return result
...
def parToLocMesh(mesh, particles):
    ...
    for p in range(count):
        pos = mc.getParticleAttr(
            ('%s.pt[%d]' %
             (particles, p)), at='worldPosition')
...
def cubeCell(obj, parent, mat, aPos):
    ...
    activeShard = mc.polyCube(ch=False, sx=1, sy=1, sz=1, w=(
        (BB[3] - BB[0]) * 2), h=((BB[4] - BB[1]) * 2), d=((BB[5] - BB[2]) * 2))
...
def cutCell(obj, mat, pos, rot, shardsGRP):
    ...
    mc.polyCut(
        ('%s.f[0:%d]' %
         (obj, tocut)), pc=(
            pos[0], pos[1], pos[2]), ro=(
            rot[0], rot[1], rot[2]), ch=False, df=True)
...
def calcMidPointMVector(posA, posB, aim, offset):
    ...
    rotate = mc.angleBetween(
        euler=True, v1=(
            aim[0], aim[1], aim[2]), v2=(
            dir.x, dir.y, dir.z))
...
def BBintersection1(BB1min, BB1max, BB2min, BB2max):
    ...
    if BB1min.x <= BB2min.x:
        outMinX = BB2min.x
    if BB1min.y >= BB2min.y:
        outMinY = BB1min.y
    if BB1min.y <= BB2min.y:
        outMinY = BB2min.y
    if BB1min.z >= BB2min.z:
        outMinZ = BB1min.z
    if BB1min.z <= BB2min.z:
        outMinZ = BB2min.z
    ...
    if BB1max.x <= BB2max.x:
        outMaxX = BB1max.x
    if BB1max.x >= BB2max.x:
        outMaxX = BB2max.x
    if BB1max.y <= BB2max.y:
        outMaxY = BB1max.y
    if BB1max.y >= BB2max.y:
        outMaxY = BB2max.y
    if BB1max.z <= BB2max.z:
        outMaxZ = BB1max.z
    if BB1max.z >= BB2max.z:
        outMaxZ = BB2max.z
...
def cubeVoro(obj, points, offset, rgb):
    ...
    for aPos in points:
        ...
        if mc.progressWindow(query=True, progress=True) >= len(points):
            break
        amount += 1
        ...
        bool = mc.polyBoolOp(
            activeShard, cutShard, op=3, ch=False, n=(
                obj + '_shard_1'))
        # mc.delete(bool, ch = True)
        mc.parent(bool, shardsGRP)

        mc.progressWindow(
            edit=True, progress=amount, status=(
                "Voronoi Shatter step %d of %d completed . . ." %
                (amount, len(points))))
        mc.refresh()
